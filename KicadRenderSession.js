...
import { KicadElementRectangle, KicadElementGrLine, KicadElementGrRect, KicadElementFpLine, KicadElementFpRect } from '@kicad-io/KicadElementStartEnd';
import { KicadElementCircle, KicadElementGrCircle, KicadElementFpCircle } from '@kicad-io/KicadElementCircle';
import { KicadElementArc, KicadElementGrArc, KicadElementFpArc } from '@kicad-io/KicadElementArc';
import { KicadElementPolyline, KicadElementBezier, KicadElementGrCurve } from '@kicad-io/KicadElementPolyline';
import { KicadElementGrPoly, KicadElementPolygon } from '@kicad-io/KicadElementPolygon';
...
import { SchematicConnectivityService } from '@kicad-layout/Connectivity';
...
import { BoardPainter, boardPaintOrder, defaultLayerState, paintHighlightOverlay } from './paint/BoardPainter';
import { SchematicPainter, defaultSchLayerState } from './paint/SchematicPainter';
import { boardBackgroundColor, styleForLayer } from './paint/LayerColors';
import { layerPaintRank } from './paint/LayerOrder';
import { buildCopperGraph, buildTrackChainGraph } from './paint/legacy/BoardCopperGraph';
...
import { buildBoardOutlineRegionNm, buildEdgeExclusionsByLayer, buildZoneFillJobs, resolveCopperLayers } from './paint/BoardZoneFill';
...
import { resolveZoneClearanceMm, computeWireBend, readBoardOrigin, SYNTHETIC_ANGLE_BASE, pointsNear, arcCircumcenter, quantizedAngle, pointLiesOnSegmentInterior, cubicBezierToPolyline } from './utils';
...
export class KicadRenderSession {
    ...
    schematicPainter = new SchematicPainter();
    ...
    get hiddenPinsVisible() { return this.schematicPainter.showHiddenPins; }
    ...
    lineMode = '90';
    get currentLineMode() { return this.lineMode; }
    setLineMode(mode) {
    ...
    annotateAutomatically = true;
    get isAnnotateAutomatically() { return this.annotateAutomatically; }
    ...
    annotateSchematic() {
        ...
        const symbols = this.schematicRoot.rootElement.findChildrenByClass(KicadElementSymbol);
        ...
        const claim = (prefix, num, libId, value, unit) => {
    ...
    highlightedBoardNetId = null;
    highlightedBoardNetName = null;
    connectivityService = new SchematicConnectivityService();
    connectivityCacheText = null;
    ...
    boardGridOrigin = new Vec2(0, 0);
    boardDrillPlaceOrigin = new Vec2(0, 0);
    ...
    boardHighlight = null;
    ratsnestLines = [];
    previewRatsnestLines = [];
    previewRatsnestNetIds = new Set();
    ...
    lastRatsnestCommitSignature = null;
    zoneDisplayMode = 'filled';
    padDisplayMode = 'filled';
    viaDisplayMode = 'filled';
    trackDisplayMode = 'filled';
    ...
    geometryDirty = true;
    activeBoardDragPerformance = null;
    lastBoardDragPerformance = null;
    lastBoardDragFastRejection = null;
    flipped = false;
    /**
    ...
    static hitTestPixelTolerance = 5;
    ...
    isBoardLayerVisible(layer) {
        const state = this.layerState.get(layer);
        return !!state && state.visible;
    }
    isSchLayerVisible(layer) {
    ...
    resize(widthPx, heightPx) {
        ...
        if (w < KicadRenderSession.minFitViewportPx
    ...
    hitTestAtScreen(screenPos) {
        ...
        const hit = this.documentType === 'schematic'
            ? (this.schScene ? hitTest(this.schScene.hitTestItems, worldPos.x, worldPos.y, tolerance, item => item.kind === 'symbol' || this.isSchLayerVisible(item.layer)) : null)
            : (this.scene ? hitTest(this.scene.hitTestItems, worldPos.x, worldPos.y, tolerance, item => item.kind === 'footprint' || this.isBoardLayerVisible(item.layer)) : null);
        if (!hit) {
            return null;
        }
    ...
    hitTestCandidatesAtScreen(screenPos, activeLayer) {
        if (this.documentType !== 'board' || !this.scene) {
            return { reduced: [], all: [] };
        }
        const worldPos = this.screenToWorld(screenPos);
        const tolerance = this.hitTestToleranceWorld();
        const raw = hitTestAll(this.scene.hitTestItems, worldPos.x, worldPos.y, tolerance, item => item.kind === 'footprint' || this.isBoardLayerVisible(item.layer));
        return {
    ...
    reduceHitCandidates(candidates, activeLayer) {
    ...
    candidateArea(item) {
        if (item.kind === 'via' && typeof item.element?.getDrill === 'function') {
            ...
            const width = item.element.getDrill()?.width;
    ...
    trackEndpointNear(paintId, screenPos) {
        if (this.documentType !== 'board' || !this.scene) {
            return null;
        }
     ...
     */
    hitTestSymbolAtScreen(screenPos) {
    ...
    hitTestLabelAtScreen(screenPos) {
        if (this.documentType !== 'schematic' || !this.schScene) {
            return null;
        }
        const worldPos = this.screenToWorld(screenPos);
     ...
     */
    hitTestRect(worldOrigin, worldCursor, mode) {
    ...
    expandBoardConnection() {
        ...
        const branchCount = (index) => graph.adjacent(index).filter(other => graph.nodes[other].itemKind === 'track').length;
        ...
        // pad belongs to.
        const toSelectableIds = (nodeIndices) => {
            const ids = new Set();
        ...
        let resultIds = null;
        for (const tier of ['junction', 'pad', 'never']) {
    ...
    get currentHighlightedBoardNetName() { return this.highlightedBoardNetName; }
    ...
    highlightBoardNetAtScreen(screenPos) {
        if (this.documentType !== 'board' || !this.scene) {
    ...
    footprintPaintIdForHit(paintId) {
        if (this.documentType !== 'board') {
            return paintId;
        }
    ...
    moveFootprintByPaintId(paintId, x, y) {
    ...
    rotateFootprintByPaintId(paintId, degrees) {
        if (this.documentType !== 'board' || !this.boardRoot || !this.scene) {
            return false;
        }
        const el = this.footprintOwnerOfHit(paintId);
    ...
    flipFootprintByPaintId(paintId) {
        if (this.documentType !== 'board' || !this.boardRoot || !this.scene) {
            return false;
        }
        const el = this.footprintOwnerOfHit(paintId);
        ...
        const currentLayer = el.getLayer();
    ...
    hitTestBoardRect(worldOrigin, worldCursor, mode) {
        if (this.documentType !== 'board' || !this.scene) {
            return [];
        }
        const minX = Math.min(worldOrigin.x, worldCursor.x);
    ...
    translateBoardSelection(ids, dx, dy) {
     ...
     */
    translateBoardDragFast(targets, dx, dy) {
        this.lastBoardDragFastRejection = null;
     ...
     */
    commitBoardDragFast(targets) {
        if (this.documentType !== 'board' || !this.boardRoot || !this.scene) {
            return;
        }
        const footprintsToRefresh = new Set();
        for (const el of targets.origins) {
    ...
    get isRatsnestVisible() { return this.ratsnestVisible; }
    beginBoardDragPerformance() {
        if (this.activeBoardDragPerformance) {
            return;
        }
    ...
    get latestBoardDragPerformance() {
        return this.lastBoardDragPerformance;
    }
    get currentZoneDisplayMode() { return this.zoneDisplayMode; }
    get currentBoardGridOrigin() { return this.boardGridOrigin; }
    get currentBoardDrillPlaceOrigin() { return this.boardDrillPlaceOrigin; }
    ...
    setBoardOrigin(kind, x, y) {
        ...
        const setup = this.boardRoot.rootElement.findFirstChildByName?.('setup');
        if (!setup) {
            return false;
        }
        const name = kind === 'grid' ? 'grid_origin' : 'aux_axis_origin';
        let origin = setup.findFirstChildByName?.(name);
        if (!origin) {
    ...
    get currentPadDisplayMode() { return this.padDisplayMode; }
    get currentViaDisplayMode() { return this.viaDisplayMode; }
    get currentTrackDisplayMode() { return this.trackDisplayMode; }
    ...
    get currentCrosshairMode() { return this.crosshairMode; }
    ...
    addTrackSegment(x1, y1, x2, y2, width, layer, netId, captureUndo = true) {
        if (this.documentType !== 'board' || !this.boardRoot?.rootElement
            || (x1 === x2 && y1 === y2)) {
            return null;
        }
    ...
    shoveTrackSegment(element, newSegments) {
    ...
    assembleTrackLine(paintId) {
        ...
        const [nodeAIndex, nodeBIndex] = ownNodeIndices;
    ...
    isTrackChainStop(graph, nodeIndex) {
        const node = graph.nodes[nodeIndex];
    ...
    walkTrackChainOutward(graph, startIndex, arrivedFromId) {
        const segmentIds = [];
        const points = [];
    ...
    dragTrackLine(oldSegmentIds, newPoints, width, layer, netId) {
        ...
        const oldElements = parent.children.filter((child) => {
        ...
        if (this.scene) {
            ...
            this.copperGraphCache = null;
            this.selectedIds = new Set(replacements.map(segment => segment.getUuid()).filter((id) => !!id));
            this.scheduleRender();
        }
        else {
            this.commitAstMutation();
    ...
    cleanupTracks() {
        ...
        const allSegments = root.findChildrenByClass(KicadElementSegment);
        ...
        let mergedCount = 0;
        const EPS = 1e-6;
        const pointKey = (layer, netId, x, y) => `${layer}|${netId}|${x.toFixed(5)}|${y.toFixed(5)}`;
        let changed = true;
        while (changed) {
            changed = false;
    ...
    commitViaDrag(paintId, fixes, x, y) {
        if (this.documentType !== 'board' || !this.boardRoot?.rootElement) {
            return false;
        }
        const parent = this.boardRoot.rootElement;
        const viaElement = parent.children.find((child) => child instanceof KicadElementVia && child.getUuid() === paintId);
        if (!viaElement) {
            return false;
        }
    ...
    applyFixChain(parent, fix, chain, width, layer, netId) {
        {
     ...
     *  fix ended up locked. */
    trackCornerDragFanout(paintId, endpoint) {
        if (this.documentType !== 'board' || !this.scene) {
            return null;
        }
        this.rebuildBoardSceneIfPending();
        ...
        const [nodeA, nodeB] = ownNodeIndices;
        ...
        const fixes = [];
        const seenTrackIds = new Set([paintId]);
    ...
    addVia(x, y, size, drill, layers, netId, captureUndo = true) {
    ...
    addBoardGraphicLine(x1, y1, x2, y2, layer, strokeWidth = 0.1) {
    ...
    addBoardGraphicRect(x1, y1, x2, y2, layer, strokeWidth = 0.1) {
        if (!this.canAddBoardGraphic() || (x1 === x2 && y1 === y2)) {
            return null;
        }
    ...
    addBoardGraphicCircle(cx, cy, radius, layer, strokeWidth = 0.1) {
    ...
    addBoardGraphicArc(sx, sy, mx, my, ex, ey, layer, strokeWidth = 0.1) {
    ...
    addBoardGraphicBezier(points, layer, strokeWidth = 0.1) {
    ...
    addBoardGraphicText(x, y, value, layer) {
    ...
    addBoardGraphicTextBox(x1, y1, x2, y2, value, layer) {
    ...
    addBoardGraphicImage(x, y, data, mimeType, layer, scale = 1) {
    ...
    addBoardDimension(type, first, second, placement, layer) {
    ...
    formatDimensionValueText(dim, measuredMm) {
    ...
    getDimensionMeasuredText(paintId) {
        if (this.documentType !== 'board' || !this.scene) {
            return null;
        }
        const dim = this.findDimensionByPaintId(paintId);
    ...
    findDimensionByPaintId(paintId) {
    ...
    getDimensionEditAnchors(paintId) {
        const dim = this.findDimensionByPaintId(paintId);
        const points = dim?.getPoints();
    ...
    moveDimensionMeasuredPoint(paintId, index, x, y) {
        const dim = this.findDimensionByPaintId(paintId);
        const points = dim?.getPoints();
    ...
    setDimensionHeightFromCursor(paintId, cursorX, cursorY) {
        const dim = this.findDimensionByPaintId(paintId);
        const points = dim?.getPoints();
        if (!dim || !points || points.length < 2) {
            return false;
    ...
    netIdAtScreen(screenPos) {
        ...
        const element = this.scene.hitTestItems.find(item => item.id === hit.id)?.element;
    ...
    netNameForId(netId) {
        ...
        if (!this.netNameCache) {
            ...
            for (const net of this.boardRoot.rootElement.findChildrenByClass(KicadElementNet)) {
                const name = net.netName;
                if (name !== undefined) {
                    this.netNameCache.set(net.id, name);
                }
            }
        }
    ...
    nearestRatsnestPoint(worldPos, toleranceWorld) {
        if (this.documentType !== 'board') {
            return null;
        }
        let best = null;
    ...
    nearestAnchorPoint(worldPos, toleranceWorld, netFilter) {
        if (this.documentType !== 'board' || !this.scene) {
            return null;
        }
        let best = null;
        const consider = (x, y, netId, kind, width) => {
     ...
     * to something."
     */
    nearestBoardGraphicAnchor(worldPos, toleranceWorld) {
        if (this.documentType !== 'board' || !this.scene) {
            return null;
        }
        let best = null;
        ...
        for (const bucket of this.scene.layerBuckets.values()) {
            for (const item of bucket) {
                ...
                if (item.kind === 'pad' || item.kind === 'via') {
                    ...
                    const origin = item.element?.getOrigin?.();
                    if (origin) {
                    ...
                    if (el instanceof KicadElementGrLine || el instanceof KicadElementFpLine ||
    ...
    highlightNetAtScreen(screenPos) {
        if (this.documentType !== 'schematic' || !this.schScene) {
    ...
    netNameForWireItem(item) {
        ...
        const addCandidateName = (candidate) => {
            ...
            if (candidate.kind === 'symbol' && candidate.refDesignator) {
                ...
                if (component) {
                    const nets = [...new Set(component.pins.map(p => p.net).filter(Boolean))];
                    if (nets.length === 1) {
                        names.add(nets[0]);
                    }
                }
        ...
        const pointsClose = (x1, y1, x2, y2) => Math.hypot(x1 - x2, y1 - y2)
            <= connectTolerance;
        const itemTouchesSegment = (candidate, segment) => {
    ...
    netNameForWireByMembership(itemId) {
        const summary = this.getConnectivitySummary();
    ...
    paintIdsForNet(netName) {
        ...
        const addLabelAndCompanions = (labelItem) => {
    ...
    allGroups() {
        ...
        return this.schematicRoot.rootElement.children.filter((c) => c instanceof KicadElementGroup);
    }
    /**
    ...
    expandGroupSelection(ids) {
        ...
        for (const id of ids) {
            const uuid = hitItems.find(it => it.id === id)?.element?.getUuid?.();
            if (!uuid) {
                continue;
            }
     ...
     *  existing group. */
    selectionHasGroup(ids) {
        const groups = this.allGroups();
        if (groups.length === 0 || !this.schScene) {
            return false;
        }
        const hitItems = this.schScene.hitTestItems;
        const uuids = new Set(ids.map(id => hitItems.find(it => it.id === id)?.element?.getUuid?.()).filter(Boolean));
     ...
     */
    fitSchematicContent(excludeLayers = ['Frame']) {
        if (!this.schScene) {
            return;
        }
    ...
    // ---- Loading ----
    async loadBoardText(text, options) {
        ...
        const setup = rootElement.findFirstChildByName?.('setup');
        ...
        this.boardConnectivity.Build(buildBoardFacadeFromAst(this.boardRoot.rootElement, this.scene));
        ...
        try {
            if (typeof Worker !== 'undefined') {
                ...
                worker.onmessage = (ev) => {
                    if (ev.data?.lines) {
                        this.ratsnestLines = ev.data.lines.map((l) => ({ from: new Vec2(l.from.x, l.from.y), to: new Vec2(l.to.x, l.to.y), netId: l.netId }));
                        this.scheduleRender();
                    }
     ...
     */
    loadFootprintPreviewText(sourceText) {
        const parsedFootprint = parseText(sourceText);
        const footprint = parsedFootprint.name === 'footprint'
            ? parsedFootprint
            : parsedFootprint.children.find((child) => child.name === 'footprint');
        if (!footprint) {
            return false;
        }
    ...
    repairDerivedLibSymbols(root) {
        const libSymbols = root?.children?.find((c) => c?.name === 'lib_symbols');
        const children = libSymbols?.children;
        if (!children) {
            return;
        }
    ...
    repairMalformedBoardZoneOutlines(root) {
        const zones = root?.findChildrenByClass?.(KicadElementZone);
        if (!zones?.length) {
            return;
        }
    ...
    repairDetachedBoardZoneFields(root) {
        const children = root?.children;
        if (!children) {
            return;
        }
        ...
        for (let index = 0; index < children.length; index++) {
            ...
            const hasOutline = zone.children?.some((child) => child.name === 'polygon');
            const hasLayer = zone.children?.some((child) => child.name === 'layer' || child.name === 'layers');
     ...
     */
    async loadSchematicText(text, docInfo) {
    ...
    async resyncSchematicFromAst(text) {
    ...
    scheduleFootprintRebuild(footprint) {
     ...
     *  incremental rebuild. */
    rebuildAfterFootprintGeometryEdit(footprint) {
     ...
     *  in the first place. */
    refreshRatsnestForFootprints(footprints) {
    ...
    beginBoardDragPreview(paintIds) {
    ...
    captureDragPreviewRatsnestEdges(footprint) {
    ...
    beginTrackDragPreview(paintIds) {
        if (this.documentType !== 'board' || !this.scene) {
            return;
        }
    ...
    findZoneByUuid(uuid) {
        ...
        const zones = this.boardRoot.rootElement.findChildrenByClass(KicadElementZone);
    ...
    findBoardPolygonByPaintId(paintId) {
        ...
        const polygon = this.boardRoot.rootElement.findChildrenByClass(KicadElementGrPoly)
    ...
    findGrPolyByUuid(uuid) {
        if (this.documentType !== 'board' || !this.boardRoot?.rootElement) {
            return null;
        }
        const bareUuid = uuid.includes(':') ? uuid.slice(0, uuid.indexOf(':')) : uuid;
        const polygons = this.boardRoot.rootElement.findChildrenByClass(KicadElementGrPoly);
    ...
    applyPolygonDraft(polygon, draft) {
    ...
    createPolygonFromOutline(points, draft) {
    ...
    updatePolygonProperties(paintId, draft) {
        const polygon = this.findGrPolyByUuid(paintId);
        if (!polygon) {
            return false;
        }
    ...
    keepoutZoneInputs(allZones) {
        const zones = allZones ?? (this.boardRoot?.rootElement.findChildrenByClass(KicadElementZone) ?? []);
        return zones
    ...
    static zoneFallbackIds = new WeakMap();
    static zoneFallbackIdCounter = 0;
    zoneJobId(zone) {
    ...
    zonePadConnectionSettings(zone) {
     ...
     */
    async fillZone(zoneUuid, runJobs, onProgress, designSettings) {
        if (this.documentType !== 'board' || !this.boardRoot?.rootElement || !this.scene) {
            return false;
        }
        ...
        const extraExclusionsByLayer = buildEdgeExclusionsByLayer(this.boardRoot, copperLayers, this.keepoutZoneInputs(), designSettings?.copperEdgeClearanceMm);
    ...
    getBoardNets() {
        ...
        return this.boardRoot.rootElement.findChildrenByClass(KicadElementNet)
    ...
    createZoneFromOutline(points, draft) {
     ...
     *  here on. */
    updateZoneProperties(paintId, draft) {
        if (this.documentType !== 'board' || !this.boardRoot?.rootElement) {
            return false;
        }
        const zone = this.findZoneByUuid(paintId);
    ...
    createRuleAreaFromOutline(points, draft) {
        if (!this.canAddBoardGraphic() || points.length < 3 || draft.layers.length === 0) {
            return null;
        }
    ...
    updateRuleAreaProperties(paintId, draft) {
        if (this.documentType !== 'board' || !this.boardRoot?.rootElement) {
            return false;
        }
        const zone = this.findZoneByUuid(paintId);
    ...
    getBoardPolygonAnchors(paintId) {
     ...
     *  mousemove frame. */
    moveBoardPolygonPoint(paintId, index, x, y) {
        const polygon = this.findBoardPolygonByPaintId(paintId);
    ...
    moveBoardPolygonEdge(paintId, edgeIndex, dx, dy) {
        const polygon = this.findBoardPolygonByPaintId(paintId);
    ...
    nearestBoardPolygonInsertion(paintId, x, y) {
        const polygon = this.findBoardPolygonByPaintId(paintId);
        const points = polygon?.points;
        if (!polygon || !points || points.length < 2) {
            return null;
     ...
     *  new vertex's index. */
    insertBoardPolygonPoint(paintId, afterIndex, x, y) {
        const polygon = this.findBoardPolygonByPaintId(paintId);
        const points = polygon?.points;
    ...
    async fillAllZones(runJobs, onProgress, designSettings) {
        if (this.documentType !== 'board' || !this.boardRoot?.rootElement || !this.scene) {
            return 0;
        }
        const zones = this.boardRoot.rootElement.findChildrenByClass(KicadElementZone);
        ...
        const boardOutlineNm = buildBoardOutlineRegionNm(this.boardRoot);
        const copperLayers = resolveCopperLayers(this.scene);
        const extraExclusionsByLayer = buildEdgeExclusionsByLayer(this.boardRoot, copperLayers, this.keepoutZoneInputs(zones), designSettings?.copperEdgeClearanceMm);
        ...
        const jobs = buildZoneFillJobs(zoneInputs, this.scene, boardOutlineNm, extraExclusionsByLayer, this.zonePriorityInputs(zones));
        const results = await runJobs(jobs, onProgress);
        if (this.documentType !== 'board' || !this.boardRoot?.rootElement) {
            return 0;
        }
        const resultsByZone = new Map();
     ...
     */
    async cancelLatestUndoSnapshot() {
     ...
     */
    mutateSymbolByPaintId(paintId, mutate) {
     ...
     */
    mutateLibSymbolForInstance(paintId, mutate) {
    ...
    findLibSymbolForInstance(paintId) {
     ...
     */
    mutateElementByPaintId(paintId, mutate) {
    ...
    isBoardElementLocked(paintId) {
        if (this.documentType !== 'board' || !this.boardRoot || !this.scene) {
            return false;
        }
        const element = this.scene.hitTestItems.find(it => it.id === paintId)?.element;
     ...
     * multi-selection.
     */
    mutateElementsByPaintIds(ids, mutate) {
     ...
     */
    moveSymbolByRef(reference, x, y, rotation, instanceId) {
        ...
        const props = typeof instance.getProperties === 'function'
    ...
    moveSheetById(paintId, x, y) {
        ...
        if (dx !== 0 || dy !== 0) {
            ...
            const props = typeof sheet.getProperties === 'function' ? sheet.getProperties() : [];
            for (const prop of props) {
                if (!prop || typeof prop.getOrigin !== 'function' || typeof prop.setOrigin !== 'function') {
                    continue;
                }
    ...
    moveSheetPinById(paintId, x, y) {
        ...
        const edgeRotation = { left: 180, right: 0, top: 90, bottom: 270 };
        const edgeJustify = { left: 'left', right: 'right', top: 'right', bottom: 'left' };
        const edges = [
     ...
     */
    autoplaceSymbolFields(reference, layout, instanceId) {
        if (this.documentType !== 'schematic' || !this.schematicRoot) {
            return false;
        }
        const instance = instanceId
     ...
     */
    moveLabelById(paintId, x, y, rotation) {
        if (this.documentType !== 'schematic' || !this.schematicRoot) {
            return false;
        }
        ...
        if (!el || typeof el.setOrigin !== 'function') {
            ...
            for (const kid of kids) {
                if ((kid?.name === 'global_label' || kid?.name === 'hierarchical_label' || kid?.name === 'label')
    ...
    getSymbolPose(reference, instanceId) {
        if (this.documentType !== 'schematic' || !this.schematicRoot) {
            return null;
        }
        const instance = instanceId
    ...
    insertRootChild(el) {
        ...
        const trailingIndex = root.children.findIndex((c) => c.name === 'sheet_instances' || c.name === 'embedded_files');
        if (trailingIndex === -1) {
    ...
    addWire(x1, y1, x2, y2, strokeWidth = 0) {
    ...
    addBus(x1, y1, x2, y2, strokeWidth = 0) {
    ...
    addWireLike(makeEl, x1, y1, x2, y2, strokeWidth, kind) {
        if (this.documentType !== 'schematic' || !this.schematicRoot?.rootElement) {
            return null;
        }
        this.pushUndoSnapshot();
    ...
    addBusEntry(x, y, dx = 2.54, dy = -2.54, strokeWidth = 0) {
    ...
    addGraphicTable(x, y, rows, columns, values) {
    ...
    addGraphicText(x, y, value, rotation = 0) {
     ...
     * one undo snapshot.
     */
    addGraphicImage(x, y, data, mimeType, scale = 1) {
    ...
    getSelectionCurveAnchors() {
        ...
        if (el.name === 'bezier' && typeof el.getPoints === 'function') {
            ...
            const kinds = ['bezier-start', 'bezier-control-1', 'bezier-control-2', 'bezier-end'];
            return {
        ...
        if (item.shape.type === 'polygon' && (el.name === 'polyline' || el.name === 'rule_area')) {
            ...
            return {
                ...
                anchors: points.map((point, index) => ({
                    kind: `polygon-vertex-${index}`, x: point.x, y: point.y
                }))
            };
        }
        return null;
    ...
    addGraphicTextBox(x1, y1, x2, y2, value) {
    ...
    addGlobalLabel(x, y, value, shape, rotation = 0) {
    ...
    addHierLabel(x, y, value, shape, rotation = 0) {
     ...
     *  heuristic. */
    addDirectiveLabel(x, y, netclassName, shape = 'round', rotation = 0) {
     ...
     */
    flattenDerivedLibSymbol(base, derived, targetLibId) {
        const flattened = parseText(base.write());
        ...
        for (const prop of derived.getProperties()) {
            ...
            const clone = parseText(prop.write());
    ...
    getLibrarySymbolUnitCount(sourceText, symbolName) {
        const parsed = parseText(sourceText);
        const candidates = parsed.name === 'symbol'
            ? [parsed]
            : parsed.children.filter(child => child instanceof KicadElementSymbol);
    ...
    addLibrarySymbolFromText(sourceText, symbolName, x, y, libIdOverride, unit = 1, reuseReference) {
        ...
        const sourceForClone = parseText(source.write());
        sourceForClone.symbolName = libId;
        let detached = parseText(sourceForClone.write());
        ...
        const propertyAt = (name, fallbackY) => {
    ...
    nextSymbolRef(base) {
        ...
        for (const kid of this.schematicRoot?.rootElement?.children ?? []) {
            if (kid?.name !== 'symbol' || typeof kid.getReference !== 'function') {
                continue;
            }
            const value = String(kid.getReference() ?? '');
    ...
    nextPowerRef(prefix) {
        let max = 0;
        const kids = this.schematicRoot?.rootElement?.children ?? [];
    ...
    addPowerRail(x, y, name, rotation = 0) {
     ...
     */
    cloneAndPlace(sourceText, dx, dy) {
        let clone;
        try {
    ...
    static PASTEABLE_ELEMENT_NAMES = new Set([
     ...
     */
    copySelectionForSystemClipboard(ids) {
     ...
     */
    pasteSystemClipboardText(text, targetX, targetY) {
     ...
     */
    duplicateSelection(ids, dx = 2.54, dy = 2.54) {
    ...
    groupSelection(ids) {
        ...
        const uuids = [];
        for (const id of ids) {
            const uuid = this.schScene.hitTestItems.find(it => it.id === id)?.element?.getUuid?.();
            if (uuid) {
                uuids.push(uuid);
            }
        }
    ...
    ungroupSelection(ids) {
        ...
        const uuids = new Set(ids.map(id => this.schScene.hitTestItems.find(it => it.id === id)?.element?.getUuid?.())
            .filter(Boolean));
        if (uuids.size === 0) {
            return 0;
        }
        ...
        const children = this.schematicRoot.rootElement.children;
        for (const group of groups) {
    ...
    deleteElements(ids) {
        ...
        const children = this.activeRoot.rootElement.children;
        let removed = 0;
     ...
     *  renderer). */
    getEmbeddedLibrarySymbolText(libId) {
        if (this.documentType !== 'schematic' || !this.schematicRoot?.rootElement) {
            return null;
        }
        const libSymbols = this.schematicRoot.rootElement.findFirstChildByClass(KicadElementLibSymbols);
    ...
    rotateSymbolBodyItemById(paintId, direction) {
        if (this.documentType !== 'schematic' || !this.schScene) {
            return null;
        }
        const el = this.schScene.hitTestItems.find(it => it.id === paintId)?.element;
    ...
    mirrorSymbolBodyItemById(paintId, axis) {
        if (this.documentType !== 'schematic' || !this.schScene) {
            return null;
        }
        const el = this.schScene.hitTestItems.find(it => it.id === paintId)?.element;
    ...
    rotateElementGeometry(el, direction) {
        ...
        if (typeof el.getPoints === 'function' && typeof el.setPoints === 'function') {
            ...
            el.setPoints(points.map((p) => angle.rotatePoint(new Vec2(p.x, p.y), origin)));
            return true;
        }
    ...
    mirrorElementGeometry(el, axis) {
        const reflect = (x, y, cx, cy) => axis === 'horizontal' ? { x: 2 * cx - x, y } : { x, y: 2 * cy - y };
        if (typeof el.getPoints === 'function' && typeof el.setPoints === 'function') {
            const points = el.getPoints();
            const pivot = points[0];
            if (!pivot) {
            ...
            el.setPoints(points.map((p) => reflect(p.x, p.y, pivot.x, pivot.y)));
            return true;
        }
        if (typeof el.getStartMidEnd === 'function' && typeof el.setStartMidEnd === 'function') {
            const { start, mid, end } = el.getStartMidEnd();
    ...
    resizeBoardElementBoundsById(id, x, y, width, height, handle) {
    ...
    translateElementGeometry(el, dx, dy) {
        ...
        if (typeof el.getPoints === 'function' && typeof el.setPoints === 'function') {
            el.setPoints(el.getPoints().map((p) => ({ x: p.x + dx, y: p.y + dy })));
        }
    ...
    translateSelection(ids, dx, dy) {
        ...
        const selectedSymbolOwners = new Set();
        for (const id of ids) {
    ...
    resizeElementBoundsById(id, x, y, width, height, handle) {
     ...
     * mode. */
    moveCurveAnchorById(id, anchor, x, y) {
        if (this.documentType !== 'schematic' || !this.schematicRoot) {
            return false;
        }
        const el = this.schScene?.hitTestItems.find(it => it.id === id)?.element;
        ...
        if ((anchor === 'circle-center' || anchor === 'circle-radius') && el.name === 'circle') {
            ...
            const next = points.map((point, pointIndex) => pointIndex === index ? { x, y } : { x: point.x, y: point.y });
            el.setPoints(next);
        }
            ...
            polyline.setPoints(points.map((point, pointIndex) => (pointIndex === index || (closedDuplicate && ((index === 0 && pointIndex === points.length - 1)
    ...
    render() {
        ...
        this.rebuildBoardSceneIfPending();
        const renderer = this.backend === 'webgl' && this.webglRenderer ? this.webglRenderer :
        ...
        try {
            ...
            const paintActive = (viewBBox, highlightSet = highlighted) => {
                if (this.documentType === 'schematic') {
                    this.schematicPainter.paint(this.schScene, renderer, this.schLayerState, highlightSet, this.highlightedNetIds, viewBBox);
                }
                else {
                    this.painter.paint(this.scene, renderer, this.layerState, this.activeBoardLayer, this.zoneDisplayMode, highlightSet, { pad: this.padDisplayMode, via: this.viaDisplayMode, track: this.trackDisplayMode }, this.highlightedBoardNetId, viewBBox);
                }
            };
            ...
            const staticHighlighted = this.documentType === 'board' ? new Set() : highlighted;
            ...
            if (this.documentType === 'board' && this.scene && renderer.beginStaticBuild) {
                paintHighlightOverlay(this.scene, renderer, this.selectedIds, { pad: this.padDisplayMode, via: this.viaDisplayMode, track: this.trackDisplayMode });
            }
    ...
    drawBoardOrigins(renderer) {
        ...
        const drawMarker = (origin, color, style) => {
    ...
    drawEditPreview(renderer) {
        ...
        switch (p.kind) {
            case 'wire': {
                ...
                renderer.line(bendPoint ? [p.from, bendPoint, p.cursor] : [p.from, p.cursor], { strokeColor: color, strokeWidth: 0.15 });
                break;
            }
            case 'line':
                ...
                break;
            case 'route':
                renderer.line([...p.points, p.cursor], { strokeColor: p.collides ? ROUTE_COLLISION_COLOR : color, strokeWidth: p.width });
                break;
            case 'via-drag':
            ...
            case 'text-box': {
                ...
                if (p.text) {
                    const geometry = computeStrokeTextGeometry(p.text, new Vec2(p.x, p.y), 1.27, 0, false, 0.15, { x: 0, y: 0 });
                    drawStrokeTextGeometry(renderer, geometry, color);
                }
                break;
            }
    ...
    drawBoardRatsnest(renderer) {
        ...
        const previewNetIds = this.previewRatsnestNetIds ?? new Set();
        // Build a map of preview lines by netId for quick lookup.
        const previewByNet = new Map();
    ...
    drawBoardDragPreview(renderer) {
        ...
        for (const items of this.dragPreviewFootprints.values()) {
            ...
            for (const item of sorted) {
                ...
                if (item.kind === 'footprint') {
                    if (highlighted) {
                        renderer.rect(new Vec2(item.bbox.x, item.bbox.y), item.bbox.w, item.bbox.h, { strokeColor: '#ffcc00', strokeWidth: 0.18 });
                    }
                    continue;
                }
    ...
    drawCommittedTrackOverlay(renderer) {
        for (const item of this.committedTrackOverlay) {
            if (!this.isBoardLayerVisible(item.layer))
                continue;
            const state = this.layerState.get(item.layer);
    ...
    drawBoardHighlight(renderer) {
        ...
        renderer.rect(new Vec2(x - pad, y - pad), w + pad * 2, h + pad * 2, { strokeColor: BOARD_HIGHLIGHT_COLOR, strokeWidth: 0.2 });
    }
    ...
    drawBoardCrosshair(renderer) {
        ...
        const style = { strokeColor: BOARD_CURSOR_COLOR, strokeWidth: lineWidth, capStyle: 'butt' };
    ...
    drawSelectionResizeHandles(renderer) {
    ...
    drawDimensionEditHandles(renderer) {
        if (this.documentType !== 'board' || this.selectedIds.size !== 1
            || !Number.isFinite(this.camera.zoom) || this.camera.zoom <= 0) {
            return;
        }
    ...
    drawSelectionCurveAnchors(renderer) {
...
import { drawCrosshair, drawLabelFlagPreview } from './editPreview';
