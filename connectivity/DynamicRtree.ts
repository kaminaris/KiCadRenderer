 ...
 */
export class DYNAMIC_RTREE<DATATYPE> {
	// Fraction of entries to reinsert on overflow (30% per R*-tree paper)
	readonly REINSERT_COUNT: number;
	readonly MINNODES: number;

	private m_root: RTREE_NODE<DATATYPE> | null = null;
	private m_count = 0;
	 ...
	 */
	Insert(aMin: number[], aMax: number[], aData: DATATYPE): void {
		if (!this.m_root) {
			this.m_root = new RTREE_NODE<DATATYPE>(this.NUMDIMS, this.MAXNODES);
			this.m_root.level = 0;
		}
	 ...
	 */
	Remove(aMin: number[], aMax: number[], aData: DATATYPE): boolean {
		if (!this.m_root) {
			return false;
		}

		// Try removal using the provided bbox first
		let reinsertList: RTREE_NODE<DATATYPE>[] = [];
		...
		const fullMin = new Array<number>(this.NUMDIMS).fill(INT64_MIN);
		const fullMax = new Array<number>(this.NUMDIMS).fill(INT64_MAX);
	 ...
	 */
	Search(aMin: number[], aMax: number[], aVisitor: (data: DATATYPE) => boolean): number {
	...
	private allocNode(): RTREE_NODE<DATATYPE> {
		return new RTREE_NODE<DATATYPE>(this.NUMDIMS, this.MAXNODES);
	}

	private freeNode(_aNode: RTREE_NODE<DATATYPE> | null): void {
		// GC handles deallocation — kept for name parity with the C++.
	}

	private removeAllNodes(aNode: RTREE_NODE<DATATYPE> | null): void {
		if (!aNode) {
			return;
		}
	...
	private insertImpl(
		...
		aMax: number[],
		aData: DATATYPE,
		reinsertedLevels: { value: number }
	): void {
		// ChooseSubtree to find the leaf
		const path: RTREE_NODE<DATATYPE>[] = [];
	...
	private chooseSubtree(
		aNode: RTREE_NODE<DATATYPE>,
		aMin: number[],
		aMax: number[],
		aPath: RTREE_NODE<DATATYPE>[]
	): RTREE_NODE<DATATYPE> {
		aPath.length = 0;
		let node = aNode;
	...
	private forcedReinsert(
		...
		interface ENTRY {
			...
			insertMax: number[];
			data: DATATYPE;
			child: RTREE_NODE<DATATYPE> | null; // Only for internal nodes
			distSq: number;
		}

		const totalEntries = aNode.count + 1;
		const entries: ENTRY[] = new Array(totalEntries);

		// Compute node center
		const nodeMin = new Array<number>(this.NUMDIMS);
		const nodeMax = new Array<number>(this.NUMDIMS);
		aNode.ComputeEnclosingBounds(nodeMin, nodeMax);

		const center = new Array<number>(this.NUMDIMS);
		...
		for (let i = 0; i < aNode.count; ++i) {
			const e: ENTRY = {
				min: new Array<number>(this.NUMDIMS),
				max: new Array<number>(this.NUMDIMS),
				insertMin: new Array<number>(this.NUMDIMS),
				insertMax: new Array<number>(this.NUMDIMS),
				data: aData,
				child: null,
				distSq: 0,
			};
	...
	private makeSplitEntry(): SPLIT_ENTRY<DATATYPE> {
		return {
			min: new Array<number>(this.NUMDIMS),
			max: new Array<number>(this.NUMDIMS),
			insertMin: new Array<number>(this.NUMDIMS),
	...
	private splitNode(
		...
		const totalEntries = aNode.count + 1;
		const entries: SPLIT_ENTRY<DATATYPE>[] = new Array(totalEntries);
		...
		const sibMin = new Array<number>(this.NUMDIMS);
		const sibMax = new Array<number>(this.NUMDIMS);
	 ...
	 */
	private computeSplitPerimeters(entries: SPLIT_ENTRY<DATATYPE>[], aTotalEntries: number): number {
		let sum = 0;
	 ...
	 */
	private findBestSplitIndex(entries: SPLIT_ENTRY<DATATYPE>[], aTotalEntries: number): number {
	 ...
	 */
	private computeSplitOverlap(
		entries: SPLIT_ENTRY<DATATYPE>[],
		aSplitIdx: number,
		aTotalEntries: number
	): number {
		const g1Min = new Array<number>(this.NUMDIMS).fill(INT64_MAX);
		const g1Max = new Array<number>(this.NUMDIMS).fill(INT64_MIN);
		const g2Min = new Array<number>(this.NUMDIMS).fill(INT64_MAX);
		const g2Max = new Array<number>(this.NUMDIMS).fill(INT64_MIN);
	 ...
	 */
	private computeOverlap(aNode: RTREE_NODE<DATATYPE>, aIdx: number): number {
		let total = 0;
		const iMin = new Array<number>(this.NUMDIMS);
		const iMax = new Array<number>(this.NUMDIMS);
	 ...
	 * other children.
	 */
	private computeOverlapEnlarged(
		aNode: RTREE_NODE<DATATYPE>,
		aIdx: number,
		aMin: number[],
		aMax: number[]
	): number {
		const enlargedMin = new Array<number>(this.NUMDIMS);
		const enlargedMax = new Array<number>(this.NUMDIMS);
	 ...
	 */
	private adjustPath(aPath: RTREE_NODE<DATATYPE>[], aBottomChild: RTREE_NODE<DATATYPE> | null = null): void {
		let childToUpdate: RTREE_NODE<DATATYPE> | null = aBottomChild;
		...
		for (let i = aPath.length - 1; i >= 0; --i) {
			...
			if (childToUpdate) {
				...
				if (slot >= 0) {
					const childMin = new Array<number>(this.NUMDIMS);
					const childMax = new Array<number>(this.NUMDIMS);
	...
	private findChildSlot(
		aParent: RTREE_NODE<DATATYPE>,
		aChild: RTREE_NODE<DATATYPE>
	): number {
	...
	private removeImpl(
		...
		aMax: number[],
		aData: DATATYPE,
		aReinsertList: RTREE_NODE<DATATYPE>[]
	): boolean {
		if (aNode.IsLeaf()) {
			for (let i = 0; i < aNode.count; ++i) {
	 ...
	 */
	private reinsertOrphans(aReinsertList: RTREE_NODE<DATATYPE>[]): void {
		for (const orphan of aReinsertList) {
			if (orphan.IsLeaf()) {
				for (let i = 0; i < orphan.count; ++i) {
					const mn = new Array<number>(this.NUMDIMS);
					const mx = new Array<number>(this.NUMDIMS);
	...
	private searchImpl(
		...
		aMin: number[],
		aMax: number[],
		aVisitor: (data: DATATYPE) => boolean,
		aFound: { value: number }
	): boolean {
...
interface SPLIT_ENTRY<DATATYPE> {
	min: number[];
	max: number[];
	insertMin: number[];
	insertMax: number[];
	data: DATATYPE;
	child: RTREE_NODE<DATATYPE> | null;
}

/** Mirrors std::countr_zero for 32-bit masks (fanout <= 16 here). */
function countrZero(mask: number): number {
	let c = 0;
