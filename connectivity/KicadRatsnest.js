...
import { buildBoardFacadeFromAst } from './KicadBoardFacade';
/**
 ...
 */
export function buildKiCadRatsnestFromBoard(boardRoot, scene, netIds) {
    try {
...
export function flattenRatsnestEdges(connData, netIds) {
    return BuildRatsnestEdges(connData, netIds);
}
export function buildKiCadRatsnest(scene, netIds) {
    try {
        const connData = new CONNECTIVITY_DATA();
 ...
 */
export function buildGreedyRatsnest(scene, netIds) {
 ...
 */
function BuildConnectivity(connData, scene) {
    const pads = [];
    const tracks = [];
    const vias = [];
    const zones = [];
 ...
 */
function BuildRatsnestEdges(connData, netIds) {
    const edges = [];
