...
import { CN_ZONE_LAYER, } from './ConnectivityItems';
/**
 * Compare two anchors by position (x, then y). Mirrors KiCad's CN_PTR_CMP.
 */
function anchorPosCompare(a, b) {
    const pa = a.Pos();
    const pb = b.Pos();
    if (pa.x === pb.x) {
        return pa.y - pb.y;
...
class TRIANGULATOR_STATE {
    ...
    Triangulate(mstEdges) {
        ...
        if (anchors.length === 0) {
            ...
            const result = delaunator(anchors.map((a) => ({ x: a.Pos().x, y: a.Pos().y })));
...
export class RN_NET {
    ...
    m_clusters = [];
    m_dirty = true;
    m_triangulator = new TRIANGULATOR_STATE();
    constructor(aNet) {
        this.m_net = aNet;
    }
    IsDirty() {
    ...
    RemoveInvalidRefs() {
        ...
        this.m_rnEdges = this.m_rnEdges.filter((edge) => edge.GetSourceNode() && edge.GetTargetNode());
        this.m_boardEdges = this.m_boardEdges.filter((edge) => edge.GetSourceNode() && edge.GetTargetNode());
    }
    /**
     ...
     */
    NearestBicoloredPair(otherNet) {
        let bestDistSq = Number.POSITIVE_INFINITY;
        let bestPos1 = null;
        let bestPos2 = null;
        const consider = (nodeA, nodeB) => {
