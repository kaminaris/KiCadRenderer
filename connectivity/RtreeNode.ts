 ...
 */
export class RTREE_NODE<DATATYPE> {
	static readonly MINNODES_FROM = (MAXNODES: number): number => Math.floor((MAXNODES * 2) / 5); // ~40%, R*-tree convention
	...
	data: DATATYPE[];
	children: (RTREE_NODE<DATATYPE> | null)[];

	constructor(private NUMDIMS: number, private MAXNODES: number) {
		...
		this.data = new Array<DATATYPE>(MAXNODES);
		this.children = new Array<RTREE_NODE<DATATYPE> | null>(MAXNODES).fill(null);
	}
	 ...
	 */
	ComputeEnclosingBounds(aMin: number[], aMax: number[]): void {
	 ...
	 */
	ChildOverlaps(i: number, aMin: number[], aMax: number[]): boolean {
		for (let d = 0; d < this.NUMDIMS; ++d) {
	 ...
	 */
	ChildOverlapMask(aMin: number[], aMax: number[]): number {
		let mask = 0;
	 ...
	 */
	ChildOverlapArea(i: number, aMin: number[], aMax: number[]): number {
		let overlap = 1;
	 ...
	 */
	ChildEnlargement(i: number, aMin: number[], aMax: number[]): number {
	 ...
	 */
	ChildPerimeter(i: number): number {
		let perimeter = 0;
	 ...
	 */
	SetChildBounds(i: number, aMin: number[], aMax: number[]): void {
		for (let d = 0; d < this.NUMDIMS; ++d) {
	 ...
	 */
	GetChildBounds(i: number, aMin: number[], aMax: number[]): void {
		for (let d = 0; d < this.NUMDIMS; ++d) {
			aMin[d] = this.bounds[d * 2]![i]!;
			aMax[d] = this.bounds[d * 2 + 1]![i]!;
		}
	 ...
	 */
	SetInsertBounds(i: number, aMin: number[], aMax: number[]): void {
		for (let d = 0; d < this.NUMDIMS; ++d) {
	 ...
	 */
	GetInsertBounds(i: number, aMin: number[], aMax: number[]): void {
		for (let d = 0; d < this.NUMDIMS; ++d) {
