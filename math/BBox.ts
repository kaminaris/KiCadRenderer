...
export class BBox {
	 ...
	 */
	static fromPoints(points: Vec2[], context?: any) {
		if (points.length == 0) {
	 ...
	 */
	constrainPoint(v: Vec2): Vec2 {
		const x = Math.min(Math.max(v.x, this.x), this.x2);
		const y = Math.min(Math.max(v.y, this.y), this.y2);
		return new Vec2(x, y);
	}

	intersectSegment(a: Vec2, b: Vec2) {
		if (this.containsPoint(a)) {
			return null;
		}

		const left: [Vec2, Vec2] = [this.topLeft, this.bottomLeft];
		const right: [Vec2, Vec2] = [this.topRight, this.bottomRight];
		const top: [Vec2, Vec2] = [this.topLeft, this.topRight];
		const bottom: [Vec2, Vec2] = [this.bottomLeft, this.bottomRight];
		...
		for (const seg of [left, right, top, bottom]) {
			...
			if (
				intersection.sub(start).squaredMagnitude <
