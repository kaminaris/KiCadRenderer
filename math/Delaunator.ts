...
export interface DelaunatorPoint {
	x: number;
	y: number;
}
...
function clockwise(px: number, py: number, qx: number, qy: number, rx: number, ry: number): boolean {
	const v0x = qx - px;
	const v0y = qy - py;
	const v1x = rx - px;
	const v1y = ry - py;
...
function counterclockwise(px: number, py: number, qx: number, qy: number, rx: number, ry: number): boolean {
	const v0x = qx - px;
	const v0y = qy - py;
	const v1x = rx - px;
	const v1y = ry - py;
...
function checkPtsEqual(x1: number, y1: number, x2: number, y2: number): boolean {
...
export interface DelaunatorResult {
	/** Triplet of point indices per triangle (index into `points`). */
	triangles: number[];
	/** Halfedge adjacency: halfedges[e] is the twin halfedge index or -1. */
	halfedges: number[];
}
...
function pointEqualRelative(ax: number, ay: number, bx: number, by: number, span: number): boolean {
	const dx = ax - bx;
	const dy = ay - by;
	const d = dx * dx + dy * dy;
	return d / span < 1e-20;
 ...
 */
export function delaunator(points: readonly DelaunatorPoint[]): DelaunatorResult {
	...
	const hash = new Array<number>(hashSize).fill(INVALID_INDEX);
	const hullPrev = new Array<number>(n);
	const hullNext = new Array<number>(n);
	const hullTri = new Array<number>(n);
	let hullStart = i0;
	let hullSize = 3;
	...
	const hashKey = (x: number, y: number): number => {
		const dx = x - center.x;
		const dy = y - center.y;
	...
	function addTriangle(iA: number, iB: number, iC: number, a: number, b: number, c: number): number {
	...
	for (let k = 0; k < n; k++) {
		...
		if (checkPtsEqual(x, y, i0x, i0y) ||
			...
			checkPtsEqual(x, y, i2x, i2y)) continue;
		...
		for (let j = 0; j < hashSize; j++) {
			...
			if (start !== INVALID_INDEX && start !== hullNext[start]) break;
		}
		...
		for (;;) {
			...
			if (counterclockwise(x, y, coords[2 * e], coords[2 * e + 1], coords[2 * q], coords[2 * q + 1])) break;
			e = q;
			if (e === start) {
				e = INVALID_INDEX;
				break;
		...
		for (;;) {
			q = hullNext[next]!;
			if (!counterclockwise(x, y, coords[2 * next], coords[2 * next + 1], coords[2 * q], coords[2 * q + 1])) break;
		...
		if (e === start) {
			for (;;) {
				q = hullPrev[e]!;
				if (!counterclockwise(x, y, coords[2 * q], coords[2 * q + 1], coords[2 * e], coords[2 * e + 1])) break;
