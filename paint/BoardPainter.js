...
import { computeStrokeTextGeometry, drawStrokeTextGeometry, getStrokeTextBounds } from './TextPaint';
import { shapeToBBox, bboxesIntersect } from './PaintedShape';
 ...
 */
export function boardPaintOrder(layersPresent, activeLayer) {
...
export function defaultLayerState(layersPresent) {
    const state = new Map();
...
export class BoardPainter {
    options = { showPadNumbers: true, showNetNames: true };
    ...
    buildZoneFills(zones, copperLayers) {
        const regions = [];
        for (const zone of zones) {
            ...
            const filledPolygons = typeof zone.getFilledPolygons === 'function' ? zone.getFilledPolygons() : [];
     ...
     */
    updateFootprintItems(scene, board, footprint) {
     ...
     */
    removeFootprintItems(scene, footprint) {
        const origin = footprint.getOrigin();
        const footprintId = footprint.getUuid() ?? `fp:${origin.x},${origin.y}`;
        const belongsToFootprint = (id) => id === footprintId || id.startsWith(`${footprintId}:`);
     ...
     */
    buildFootprintPreviewItems(board, footprint) {
    ...
    updateTrackItems(scene, board, oldIds, segments) {
        const allOldItemIds = new Set();
     ...
     */
    paint(scene, renderer, layerState, activeLayer = null, zoneDisplayMode = 'filled', highlightedIds = new Set(), itemDisplayModes = { pad: 'filled', via: 'filled', track: 'filled' }, 
    ...
    getGlobalLayerNames(board) {
        const layersEl = board.rootElement.findFirstChildByClass(getLayersClass());
        if (!layersEl) {
            return [];
        }
        return (layersEl.layers ?? []).map((l) => l.name);
    }
    buildTrack(segment, board) {
        ...
        const shape = { type: 'segment', x1: start.x, y1: start.y, x2: end.x, y2: end.y, width };
        const items = [{
    ...
    buildTrackMaskItem(id, layer, start, end, width, element) {
        const shape = { type: 'segment', x1: start.x, y1: start.y, x2: end.x, y2: end.y, width };
        return {
            id: `${id}:${layer}`, layer, kind: 'graphic', shape, bbox: shapeToBBox(shape),
            hitTestable: false, element,
            draw: (renderer, color) => renderer.line([new Vec2(start.x, start.y), new Vec2(end.x, end.y)], { strokeColor: color, strokeWidth: width }),
        };
    }
    ...
    buildTrackArc(arc, board) {
        ...
        const shape = { type: 'circle', cx: centerX, cy: centerY, r: radius, filled: false, strokeWidth: width };
        const items = [{
        ...
        if (maskLayer) {
            ...
            items.push({
                ...
                draw: (renderer, color) => renderer.arc(new Vec2(centerX, centerY), radius, startAngle, endAngle, { strokeColor: color, strokeWidth: maskWidth }),
            });
        }
        return items;
    }
    buildZone(zone) {
        ...
        const layers = typeof zone.getLayers === 'function' ? zone.getLayers() :
    ...
    buildFootprint(footprint, globalLayers, board) {
        const items = [];
        const origin = footprint.getOrigin();
        const footprintId = footprint.getUuid() ?? `fp:${origin.x},${origin.y}`;
        const footprintLayer = typeof footprint.getLayer === 'function' ? footprint.getLayer() : 'F.Cu';
        ...
        if (typeof footprint.getVisibleProperties === 'function') {
            ...
            for (const name of Object.keys(visibleProps)) {
                ...
                const propLayer = typeof prop.getLayer === 'function' ? prop.getLayer() : (isBack ? 'B.SilkS' : 'F.SilkS');
        ...
        const shape = { type: 'polygon', points: fallbackHull };
        items.unshift({
            id: footprintId,
     ...
     */
    buildTextElement(textEl, footprintMatrix, footprintId, footprintRotation = 0, 
    ...
    footprint) {
        ...
        const resolvedValue = footprint ? expandTextVars(textEl.value, footprintTextVars(footprint)) : textEl.value;
        if (!resolvedValue) {
            return null;
        }
        const layer = typeof textEl.getLayer === 'function' ? textEl.getLayer() : 'F.SilkS';
        ...
        if (cacheRings) {
            ...
            return {
                ...
                draw: (renderer, color) => {
                    if (knockout) {
                        const margin = 0.3;
                        renderer.rect(new Vec2(bbox.x - margin, bbox.y - margin), bbox.w + margin * 2, bbox.h + margin * 2, { fillColor: color });
    ...
    buildFpLine(line, footprintMatrix, footprintId) {
        ...
        const shape = { type: 'segment', x1: worldStart.x, y1: worldStart.y, x2: worldEnd.x, y2: worldEnd.y, width };
        return {
    ...
    buildFpRect(rect, footprintMatrix, footprintId) {
        ...
        const shape = { type: 'polygon', points: worldCorners.map(p => ({ x: p.x, y: p.y })) };
        return {
    ...
    buildFpCircle(circle, footprintMatrix, footprintId) {
        ...
        const shape = { type: 'circle', cx: worldCenter.x, cy: worldCenter.y, r: localRadius };
        return {
    ...
    buildFpArc(arc, footprintMatrix, footprintRotationDeg, footprintId) {
        if (typeof arc.getArcCenterRadiusAngles !== 'function') {
            return null;
        }
        ...
        const shape = { type: 'circle', cx: worldCenter.x, cy: worldCenter.y, r: radius };
        return {
            ...
            draw: (renderer, color) => {
                renderer.arc(worldCenter, radius, startAngle + rotationRad, endAngle + rotationRad, { strokeColor: color, strokeWidth: width || 0.1 });
            },
        };
    }
    ...
    buildFpPoly(poly, footprintMatrix, footprintId) {
        const points = typeof poly.getPoints === 'function' ? poly.getPoints() : [];
        if (points.length < 3) {
            return null;
        }
        ...
        const shape = { type: 'polygon', points: worldPoints.map(point => ({ x: point.x, y: point.y })) };
        // Namespaced under footprintId — see buildFpLine's doc comment.
    ...
    buildPad(pad, footprintMatrix, footprintRotationDeg, footprint, footprintId, globalLayers, board) {
        ...
        const padLayers = typeof pad.getLayers === 'function' ? pad.getLayers(globalLayers) : ['F.Cu'];
        const buckets = [];
        ...
        if (pad.shape === 'circle') {
            const shape = { type: 'circle', cx: worldCenter.x, cy: worldCenter.y, r: size.width / 2 };
            ...
            if (pad.shape === 'roundrect' || pad.shape === 'oval') {
                const rratio = pad.shape === 'oval'
                    ...
                    : (typeof pad.getSimpleChildValue === 'function'
                        ? (pad.getSimpleChildValue('roundrect_rratio') ?? 0)
                        : 0);
        ...
        items.push(...this.buildPadTechnicalItems(pad, footprint, board, fullMatrix, id, size, padLayers));
        if (this.options.showPadNumbers || this.options.showNetNames) {
            const labelItem = this.buildPadNumber(pad, worldCenter, size, footprintRotationDeg, padRotationDeg, id);
            if (labelItem) {
     ...
     */
    buildPadTechnicalItems(pad, footprint, board, fullMatrix, padId, size, padLayers) {
        const items = [];
        ...
        for (const layer of technicalLayers) {
            ...
            const common = { id, layer, kind: 'graphic', hitTestable: false, element: pad };
            if (pad.shape === 'custom') {
                ...
                const shape = { type: 'polygon', points: rings[0].map(point => ({ x: point.x, y: point.y })) };
                items.push({
            ...
            if (pad.shape === 'circle' && Math.abs(apertureWidth - apertureHeight) < 1e-9) {
                ...
                const shape = { type: 'circle', cx: center.x, cy: center.y, r: apertureWidth / 2 };
                items.push({
                    ...common, shape, bbox: shapeToBBox(shape),
    ...
    static MAX_PAD_LABEL_FONT_SIZE = 10;
    /**
     ...
     */
    buildPadNumber(pad, worldCenter, size, footprintRotationDeg, padRotationDeg, padId) {
        const padNumber = this.options.showPadNumbers ? String(pad.padNumber ?? '') : '';
        const netName = this.options.showNetNames && typeof pad.getNetName === 'function'
            ? (pad.getNetName() ?? '') : '';
        ...
        const offsetPoint = (localY) => new Vec2(worldCenter.x + localY * sin, worldCenter.y + localY * cos);
        const parts = [];
        let half = 0;
        if (showNumber && numberFontSize > 0) {
            const strokeWidth = numberFontSize / 6;
            const geometry = computeStrokeTextGeometry(padNumber, offsetPoint(numberY), numberFontSize, worldAngle, false, strokeWidth, { x: 0.5, y: 0.5 });
            parts.push({ geometry });
            half = Math.max(half, numberFontSize);
        }
        if (showNet && netFontSize > 0) {
            const strokeWidth = netFontSize / 6;
            const geometry = computeStrokeTextGeometry(netName, offsetPoint(netY), netFontSize, worldAngle, false, strokeWidth, { x: 0.5, y: 0.5 });
            parts.push({ geometry });
    ...
    buildVia(via, board) {
        ...
        const shape = { type: 'circle', cx: origin.x, cy: origin.y, r: outerRadius };
        const items = [{
        ...
        for (const layer of getViaMaskLayers(via, board, viaLayers)) {
            ...
            const maskShape = { type: 'circle', cx: origin.x, cy: origin.y, r: radius };
            items.push({
    ...
    buildViaLabel(via, origin, outerDiameter, viaId) {
        if (!this.options.showNetNames || typeof via.getNetName !== 'function') {
            return null;
        }
        const netName = via.getNetName() ?? '';
        if (!netName) {
            return null;
        }
    ...
    buildGrRect(rect) {
        ...
        const filled = fill === true || fill === 'yes' || fill === 'solid';
        const shape = { type: 'rect', x, y, w, h, filled, strokeWidth: width };
        return {
    ...
    buildDimension(dim) {
        ...
        const points = typeof dim.getPoints === 'function' ? dim.getPoints() : [];
        const height = typeof dim.getHeight === 'function' ? (dim.getHeight() ?? 0) : 0;
        const dimType = typeof dim.getDimensionType === 'function' ? dim.getDimensionType() : 'orthogonal';
        const orientation = typeof dim.getOrientation === 'function' ? (dim.getOrientation() ?? 0) : 0;
        const items = [];
    ...
    buildGrCircle(circle) {
        ...
        const filled = fill === true || fill === 'yes' || fill === 'solid';
        const shape = { type: 'circle', cx: center.x, cy: center.y, r: radius, filled, strokeWidth: width };
        return {
    ...
    buildPcbTextBox(textBox, footprintMatrix, footprintId, footprintRotation = 0, hitTestable = true, forceBorder) {
        if (typeof textBox.getStartEnd !== 'function')
            return null;
        ...
        const rotate = (point) => rotateAround(point, center, angleRad);
        const toWorld = (point) => footprintMatrix ? footprintMatrix.transform(point) : point;
        const worldCorners = [
            new Vec2(x, y), new Vec2(x + width, y), new Vec2(x + width, y + height), new Vec2(x, y + height),
        ].map(rotate).map(toWorld);
        const localTextPosition = rotate(new Vec2(x + marginLeft + contentWidth * justify.x, y + marginTop + contentHeight * justify.y));
        ...
        return {
            ...
            draw: (renderer, color) => {
                if (border)
                    renderer.polygon(worldCorners, { strokeColor: color, strokeWidth: strokeWidth || 0.1 });
                if (geometry)
                    drawStrokeTextGeometry(renderer, geometry, color);
            },
        };
    ...
    buildTable(table, footprintMatrix = null, footprintId, footprintRotation = 0) {
        ...
        const enabled = (owner, name) => readChildValue(owner, name) === true || readChildValue(owner, name) === 'yes';
        const strokeWidth = (owner) => Number(owner?.findFirstChildByName?.('stroke')?.getWidth?.() ?? 0.1) || 0.1;
        ...
        for (let index = 0; index < cells.length; index++) {
            ...
            if (typeof cell.getStartEnd !== 'function')
                continue;
        ...
        if (!cellData.length)
            return items;
        const lineSegments = [];
        const addLine = (a, b, width) => lineSegments.push({ a: toWorld(a), b: toWorld(b), width });
        ...
        items.push({
            ...
            draw: (renderer, color) => {
                for (const line of lineSegments)
                    renderer.line([line.a, line.b], { strokeColor: color, strokeWidth: line.width });
            },
        });
        return items;
    }
    ...
    buildBoardImage(image, boardVersion, footprintMatrix = null, footprintId) {
        const data = typeof image.getData === 'function' ? image.getData() : undefined;
        if (!data)
            return null;
        ...
        const corners = (footprintMatrix ? localCorners.map(point => footprintMatrix.transform(point)) : localCorners);
    ...
    buildBarcode(barcode) {
        ...
        if (!encoding || encoding.width <= 0 || encoding.height <= 0)
            return null;
        ...
        const transform = (x, y) => new Vec2(origin.x + x * cos + y * sin, origin.y - x * sin + y * cos);
        const rectangles = encoding.rectangles.map(rect => {
            ...
            return [transform(x - halfWidth, y - halfHeight), transform(x + halfWidth, y - halfHeight), transform(x + halfWidth, y + halfHeight), transform(x - halfWidth, y + halfHeight)];
        });
        ...
        return {
            ...
            draw: (renderer, color) => {
                if (knockout)
                    renderer.rect(new Vec2(bbox.x, bbox.y), bbox.w, bbox.h, { fillColor: color });
                for (const rectangle of rectangles)
                    renderer.polygon(rectangle, { fillColor: knockout ? boardBackgroundColor : color });
                if (textGeometry)
                    drawStrokeTextGeometry(renderer, textGeometry, knockout ? boardBackgroundColor : color);
            },
        };
    }
    ...
    buildTarget(target) {
        ...
        const endpoint = (sign) => new Vec2(origin.x + sign * arm * Math.cos(rotate), origin.y + sign * arm * Math.sin(rotate));
        const perpendicular = (sign) => new Vec2(origin.x - sign * arm * Math.sin(rotate), origin.y + sign * arm * Math.cos(rotate));
        ...
        const shape = { type: 'rect', x: origin.x - size / 2, y: origin.y - size / 2, w: size, h: size };
        return {
    ...
    buildPoint(point, footprintMatrix, footprintId) {
        ...
        const id = footprintId ? `${footprintId}:${rawId}` : rawId;
        const shape = { type: 'rect', x: center.x - half, y: center.y - half, w: size, h: size };
        return {
    ...
    buildGrPoly(polygon) {
        const points = typeof polygon.getPoints === 'function' ? polygon.getPoints() : [];
        if (points.length < 3)
            return null;
        ...
        return {
            ...
            draw: (renderer, color) => filled
                ...
                : renderer.line([...points.map(point => new Vec2(point.x, point.y)), new Vec2(points[0].x, points[0].y)], { strokeColor: color, strokeWidth: width || 0.1 }),
        };
    }
    buildGrCurve(curve) {
        return this.buildCurve(curve, null);
    }
    buildCurve(curve, footprintMatrix, footprintId) {
        const points = typeof curve.getPoints === 'function' ? curve.getPoints() : [];
        if (points.length !== 4)
            return null;
        ...
        const id = footprintId ? `${footprintId}:${rawId}` : rawId;
        const shapePoints = cubicBezierToPolyline(points.map(point => new Vec2(point.x, point.y)));
        ...
        const shape = { type: 'polygon', points: worldPoints.map(point => ({ x: point.x, y: point.y })), filled: false, closed: false, strokeWidth: width };
        return {
    ...
    buildEllipse(ellipse, isArc, footprintMatrix = null, footprintId) {
        ...
        if (!Number.isFinite(center.x) || !Number.isFinite(center.y) || !(major > 0) || !(minor > 0))
            return null;
        ...
        const shape = { type: 'polygon', points: points.map(point => ({ x: point.x, y: point.y })), filled, closed: !isArc, strokeWidth: width };
        return {
...
function cubicBezierToPolyline(points, steps = 32) {
    const [p0, p1, p2, p3] = points;
    const result = [];
    for (let i = 0; i <= steps; i++) {
        const t = i / steps;
        const u = 1 - t;
        result.push(new Vec2(u * u * u * p0.x + 3 * u * u * t * p1.x + 3 * u * t * t * p2.x + t * t * t * p3.x, u * u * u * p0.y + 3 * u * u * t * p1.y + 3 * u * t * t * p2.y + t * t * t * p3.y));
    }
    return result;
}
function boardNetIdsByName(board) {
 ...
 */
function arrowheadSegments(tip, awayFrom, length) {
 ...
 */
function roundedRectLocalPoints(width, height, radius, segmentsPerCorner = 6) {
 ...
 */
function buildZoneEdgeHatches(outline, pitch) {
    ...
    for (let index = 0; index < points.length; index++) {
        ...
        for (let distance = pitch * 0.5; distance < length; distance += pitch) {
            ...
            let hatchEnd = new Vec2(hatchStart.x + diagonalX * hatchLength, hatchStart.y + diagonalY * hatchLength);
            ...
            if (!pointIsInsidePolygon(hatchEnd, points)) {
                for (let iteration = 0; iteration < 8; iteration++) {
                    hatchLength *= 0.5;
                    const candidate = new Vec2(hatchStart.x + diagonalX * hatchLength, hatchStart.y + diagonalY * hatchLength);
...
function footprintHullPoints(items) {
    const points = [];
    for (const item of items) {
        ...
        switch (item.shape.type) {
            case 'rect':
                points.push({ x: item.shape.x, y: item.shape.y }, { x: item.shape.x + item.shape.w, y: item.shape.y }, { x: item.shape.x + item.shape.w, y: item.shape.y + item.shape.h }, { x: item.shape.x, y: item.shape.y + item.shape.h });
                break;
            case 'segment': {
                ...
                points.push({ x: item.shape.x1 + offsetX, y: item.shape.y1 + offsetY }, { x: item.shape.x1 - offsetX, y: item.shape.y1 - offsetY }, { x: item.shape.x2 + offsetX, y: item.shape.y2 + offsetY }, { x: item.shape.x2 - offsetX, y: item.shape.y2 - offsetY });
                break;
            }
            case 'circle':
                for (let i = 0; i < 16; i++) {
...
function isPadTechnicalLayer(layer) {
...
function readChildValue(element, name) {
...
function readChildNumber(element, name) {
    const value = readChildValue(element, name);
    const numeric = typeof value === 'number' ? value : Number(value);
    return Number.isFinite(numeric) ? numeric : undefined;
}
function getElementLayers(element) {
    const child = typeof element?.findFirstChildByName === 'function'
        ? element.findFirstChildByName('layers')
        : undefined;
    return child?.attributes?.map((attribute) => String(attribute.value)) ?? [];
}
function getCopperItemLayer(element) {
...
function getSolderMaskLayer(element, copperLayer) {
    const layers = getElementLayers(element);
    if (layers.includes('F.Mask') && copperLayer === 'F.Cu')
        return 'F.Mask';
    if (layers.includes('B.Mask') && copperLayer === 'B.Cu')
        return 'B.Mask';
    return null;
}
function getTrackMaskExpansion(track, board) {
...
function getBoardMaskExpansion(board) {
...
function getPadTechnicalMargin(pad, footprint, board, padLayers, size, layer) {
...
function readNestedBoolean(element, group, side) {
...
function getViaMaskLayers(via, board, layers) {
    const result = [];
    ...
    const backTented = readNestedBoolean(via, 'tenting', 'back')
        ...
        ?? false;
    if (layers.includes('F.Cu') && !frontTented)
        result.push('F.Mask');
    if (layers.includes('B.Cu') && !backTented)
        result.push('B.Mask');
    return result;
}
function ellipsePolyline(center, major, minor, rotation, start, end, steps) {
    const points = [];
...
let _Footprint, _Segment, _Via, _Pad, _Zone, _Layers, _GrLine, _GrVector, _GrArc, _GrRect, _GrCircle, _GrPoly, _GrCurve, _GrEllipse, _GrEllipseArc;
let _FpLine, _FpRect, _FpCircle, _FpArc, _FpPoly, _FpCurve, _FpEllipse, _FpEllipseArc, _Dimension, _GrText, _GrTextBox, _FpText, _FpTextBox, _Table, _TrackArc, _Image, _Target, _Point, _Barcode;
...
function footprintTextDrawAngle(text, footprintRotation) {
 ...
 */
function getCustomPadLocalRings(pad) {
    if (typeof pad.getCustomPolygonPoints === 'function') {
        return pad.getCustomPolygonPoints()
            .filter(r => r.length >= 3)
 ...
 */
function offsetCustomPadLocalRings(rings, margin) {
...
function footprintTextVars(footprint) {
    const vars = typeof footprint.getAllProperties === 'function'
 ...
 */
export function paintHighlightOverlay(scene, renderer, highlightedIds, itemDisplayModes = { pad: 'filled', via: 'filled', track: 'filled' }) {
 ...
 */
function getRenderCacheRings(textEl) {
