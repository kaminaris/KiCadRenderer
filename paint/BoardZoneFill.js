...
import { Area, FillRule, Point64Of, PointInPolygon, PointInPolygonResult } from '@clipper2-ts/core';
...
export function toClipperPath(points) {
...
function tessellateCircleMm(cx, cy, r, segments = 48) {
    const points = [];
...
function arcCenterlineMm(cx, cy, r, startAngle, endAngle, segments = 24) {
    const points = [];
    for (let i = 0; i <= segments; i++) {
...
function polylineCapsuleRingMm(points, width) {
    if (points.length === 0)
        return [];
    if (points.length === 1)
        return tessellateCircleMm(points[0].x, points[0].y, width / 2);
...
function trackArcRingMm(element, fallbackShape) {
...
function resolvePadConnectionMode(mode, pad) {
    const override = typeof pad?.getZoneConnectionOverride === 'function' ? pad.getZoneConnectionOverride() : null;
 ...
 */
function pushPadExclusionRing(ring, pad, zoneClearanceMm, exclusionRingsMm, preInflatedExclusionRingsMm) {
    const localClearanceMm = typeof pad?.getClearanceOverride === 'function' ? pad.getClearanceOverride() : null;
...
function inflateClosedRingMm(ring, amountMm) {
    if (amountMm <= 0)
        return ring;
    const inflated = getClipperEngine().inflatePaths([toClipperPath(ring)], amountMm * NM_PER_MM, JoinType.Round, EndType.Polygon);
...
function padThermalSpokesMm(worldCenter, localSizeMm, rotationDeg, isCircular, gapMm, spokeWidthMm, minThicknessMm) {
    const spokeWidth = Math.min(spokeWidthMm, localSizeMm.width, localSizeMm.height);
    if (spokeWidth <= 0 || spokeWidth < minThicknessMm)
        return [];
    ...
    const toWorld = (p) => { const w = matrix.transform(p); return { x: w.x, y: w.y }; };
...
function assembleClosedLoops(edges, tolerance = 0.01) {
    ...
    while (remaining.length > 0) {
        ...
        while (extended) {
            ...
            if (chain.length > 2 && pointsClose(last, chain[0], tolerance))
                break;
        ...
        const last = chain[chain.length - 1];
        if (chain.length >= 3 && pointsClose(last, chain[0], tolerance))
            loops.push(chain);
    }
    return loops;
}
...
function footprintTransform(footprint) {
    if (typeof footprint.getOrigin !== 'function')
...
function collectEdgeCutsFromElement(el, transform, closedLoops, openEdges) {
    if (typeof el.getLayer !== 'function' || el.getLayer() !== 'Edge.Cuts')
        return;
    switch (el.name) {
        case 'gr_line':
        case 'fp_line': {
        ...
        case 'fp_rect': {
            ...
            for (let i = 0; i < 4; i++)
                openEdges.push([corners[i], corners[(i + 1) % 4]]);
            break;
        }
        case 'gr_arc':
        case 'fp_arc': {
        ...
        case 'gr_circle':
        case 'fp_circle': {
            if (typeof el.getCenter !== 'function' || typeof el.getEnd !== 'function')
                break;
        ...
        case 'fp_poly': {
            if (typeof el.getPoints !== 'function')
                break;
            const pts = el.getPoints();
            if (pts.length >= 3)
                closedLoops.push(pts.map(p => transform({ x: p.x, y: p.y })));
            break;
        }
    }
}
...
function collectEdgeCutsGeometry(board) {
    const children = board?.rootElement?.children ?? [];
    const closedLoops = [];
    const openEdges = [];
 ...
 */
function buildBoardOutlineRegion(board) {
 ...
 */
export function buildBoardOutlineRingsMm(board) {
    return buildBoardOutlineRegion(board)?.map(fromClipperPath) ?? [];
}
export function buildBoardOutlineRegionNm(board) {
 ...
 */
export function buildEdgeExclusionsByLayer(board, copperLayers, keepoutZones = [], edgeClearanceMm = DEFAULT_EDGE_CLEARANCE_MM) {
    const { closedLoops, openEdges } = collectEdgeCutsGeometry(board);
    const edgeRingsMm = [];
...
export function resolveCopperLayers(scene) {
 ...
 *    knockout at all).
 */
export function buildZonePriorityKnockouts(zone, layer, copperLayers, allZones) {
    const differentNetKnockoutRingsMm = [];
    const sameNetKnockoutRingsMm = [];
    for (const other of allZones) {
        ...
        const inflated = getClipperEngine().inflatePaths([toClipperPath(other.outlinePoints)], gapMm * NM_PER_MM, JoinType.Round, EndType.Polygon);
        for (const path of inflated)
            differentNetKnockoutRingsMm.push(fromClipperPath(path));
    }
...
function isPointInsideRings(pt, rings) {
    let count = 0;
    for (const ring of rings) {
        if (PointInPolygon(pt, ring) !== PointInPolygonResult.IsOutside)
            count++;
    }
    return count % 2 === 1;
}
...
function pruneThermalSpokes(candidates, fillNm) {
 ...
 */
function applyIslandRemoval(fragments, sameNetPadRingsMm, islandRemoval) {
    ...
    const touchesPad = fragments.map(fragment => engine.booleanOp(ClipType.Intersection, FillRule.NonZero, [fragment], padRingsNm).length > 0);
...
export function computeFillFromRings(outlinePoints, exclusionRingsMm, clearanceMm, boardOutlineNm, 
...
sameNetPadRingsMm, islandRemoval) {
    if (outlinePoints.length < 3)
        return [];
    const engine = getClipperEngine();
    let outlineRingsNm = [toClipperPath(outlinePoints)];
    if (boardOutlineNm && boardOutlineNm.length > 0) {
        ...
        if (outlineRingsNm.length === 0)
            return []; // zone doesn't overlap the board at all
    }
 ...
 */
export function collectExclusionRingsMm(scene, layer, zoneNetId, 
...
padConnection) {
    ...
    const exclusionRingsMm = [];
    const preInflatedExclusionRingsMm = [];
    const spokeCandidates = [];
    const sameNetPadRingsMm = [];
    for (const item of bucket) {
        if (!isObstacleKind(item.kind))
            continue;
        ...
        if (item.netId != null && zoneNetId != null && item.netId === zoneNetId) {
            if (item.kind === 'pad') {
                ...
                if (padRing && padRing.length >= 3)
                    sameNetPadRingsMm.push(padRing);
            }
            if (item.kind !== 'pad' || !padConnection)
                continue;
            ...
            if (moat && moat.length >= 3)
                preInflatedExclusionRingsMm.push(moat);
            ...
            spokeCandidates.push(...padThermalSpokesMm(worldCenter, size, rotationDeg, isCircular, padConnection.thermalGapMm, padConnection.thermalSpokeWidthMm, padConnection.minThicknessMm));
            continue;
        }
 ...
 */
export function computeZoneFillForLayer(outlinePoints, zoneNetId, layer, scene, clearanceMm, boardOutlineNm, extraExclusionRingsMm, padConnection, 
...
sameNetKnockoutRingsMm, islandRemoval) {
    const { exclusionRingsMm, preInflatedExclusionRingsMm, spokeCandidates, sameNetPadRingsMm } = collectExclusionRingsMm(scene, layer, zoneNetId, clearanceMm, padConnection);
    return computeFillFromRings(outlinePoints, exclusionRingsMm, clearanceMm, boardOutlineNm, [...(extraExclusionRingsMm ?? []), ...preInflatedExclusionRingsMm], spokeCandidates, sameNetKnockoutRingsMm, sameNetPadRingsMm, islandRemoval);
}
/**
 ...
 */
export function buildZoneFillJobs(zones, scene, boardOutlineNm, 
...
allZonesForPriority) {
    ...
    const priorityZones = allZonesForPriority ?? zones.map((z) => ({
    ...
    for (const zone of zones) {
        if (zone.outlinePoints.length < 3)
            continue;
        const layers = zone.layers
            .flatMap(layer => layer === '*.Cu' ? copperLayers : [layer])
            .filter(layer => layer.endsWith('.Cu'));
        for (const layer of layers) {
            const { exclusionRingsMm, preInflatedExclusionRingsMm, spokeCandidates, sameNetPadRingsMm } = collectExclusionRingsMm(scene, layer, zone.netId, zone.clearanceMm, zone.padConnection);
            const { differentNetKnockoutRingsMm, sameNetKnockoutRingsMm } = buildZonePriorityKnockouts({ uuid: zone.uuid, netId: zone.netId, priority: zone.priority ?? 0, clearanceMm: zone.clearanceMm }, layer, copperLayers, priorityZones);
            jobs.push({
                zoneUuid: zone.uuid,
                layer,
...
export function runZoneFillJob(job) {
    const fills = computeFillFromRings(job.outlinePoints, job.exclusionRingsMm, job.clearanceMm, job.boardOutlineNm, job.extraExclusionRingsMm, job.spokeCandidates, job.sameNetKnockoutRingsMm, job.sameNetPadRingsMm, job.islandRemoval ?? undefined);
...
function bridgeHoleInto(outline, hole) {
    ...
    for (let i = 0; i < outline.length; i++) {
        ...
        if (holePt.y < yMin || holePt.y > yMax || yMin === yMax)
            continue;
...
function fractureFillResult(rings) {
    if (rings.length <= 1)
        return rings;
    const depth = rings.map((ring, i) => {
        ...
        for (let j = 0; j < rings.length; j++) {
            if (j !== i && PointInPolygon(testPt, rings[j]) !== PointInPolygonResult.IsOutside)
                d++;
        }
        return d;
    });
    ...
    for (const holeIdx of holeIndices) {
        ...
        if (ownerIdx === -1)
            continue; // orphaned hole (shouldn't happen) — drop
        if (!holesByOuter.has(ownerIdx))
            holesByOuter.set(ownerIdx, []);
 ...
 */
export function computeZoneFill(outlinePoints, zoneNetId, requestedLayers, scene, clearanceMm, boardOutlineNm, extraExclusionRingsMm, padConnection, 
...
sameNetKnockoutRingsMm) {
    ...
    const result = [];
    for (const layer of layers) {
        for (const ring of computeZoneFillForLayer(outlinePoints, zoneNetId, layer, scene, clearanceMm, boardOutlineNm, extraExclusionRingsMm, padConnection, sameNetKnockoutRingsMm)) {
            if (ring.length >= 3)
                result.push({ layer, points: ring });
        }
    }
    return result;
}
