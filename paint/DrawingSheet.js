...
function rect(sx, sy, ex, ey, opts = {}) {
    return {
        kind: 'rect',
        start: new Vec2(sx, sy), startAnchor: opts.startAnchor ?? 'rbcorner',
        end: new Vec2(ex, ey), endAnchor: opts.endAnchor ?? 'rbcorner',
 ...
 * closest). */
export function resolveWksAnchor(sheetSize, setup, anchor, point) {
...
export function withinWksMargin(sheetSize, setup, p) {
 ...
 * nothing.
 */
export function expandTextVars(str, vars, symbolFields) {
...
function expandTextVarsOnce(str, vars, symbolFields) {
    return str.replace(/\\\$\{([^}]*)\}|\$\{([^}]*)\}|\\\$/g, (whole, escapedName, name) => {
        if (whole === '\\$') {
            return '$';
        }
