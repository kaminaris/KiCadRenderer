...
export function embeddedImageInfo(data) {
    const byteAt = (index) => index < data.length ? data.charCodeAt(index) & 0xff : 0;
    const be16 = (index) => (byteAt(index) << 8) | byteAt(index + 1);
    const be32 = (index) => ((byteAt(index) * 0x1000000) + (byteAt(index + 1) << 16) + (byteAt(index + 2) << 8)
        + byteAt(index + 3)) >>> 0;
    const le16 = (index) => byteAt(index) | (byteAt(index + 1) << 8);
