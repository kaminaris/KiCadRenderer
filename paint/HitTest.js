import { shapeContainsPoint } from './PaintedShape';
/**
 ...
 */
export function hitTest(items, worldX, worldY, tolerance = 0, isVisible) {
 ...
 */
export function hitTestAll(items, worldX, worldY, tolerance = 0, isVisible) {
    const matches = [];
    for (let i = items.length - 1; i >= 0; i--) {
        const item = items[i];
        if (isVisible && !isVisible(item)) {
