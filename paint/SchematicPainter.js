...
import { shapeToBBox, distanceToSegment, polygonEdgeDistance, bboxesIntersect } from './PaintedShape';
import { arcToPolyline, circleToRing, strokeDashedPolyline } from './StrokeDash';
import { embeddedImageInfo } from './EmbeddedImage';
import { defaultWksItems, defaultWksSetup, expandTextVars, resolveWksAnchor, withinWksMargin, wksPaperSizes } from './DrawingSheet';
...
export function defaultSchLayerState(layersPresent) {
    const state = new Map();
...
export class SchematicPainter {
     ...
     */
    findSymbolInstanceByReference(schematic, reference) {
        const root = schematic.rootElement;
        if (!getSymbolClass()) {
            return null;
        }
     ...
     */
    buildDrawingSheet(root, docInfo) {
        const items = [];
        const paperEl = typeof root.findFirstChildByName === 'function' ? root.findFirstChildByName('paper') : null;
        const paperName = paperEl?.attributes?.[0]?.value ?? 'A4';
        ...
        for (const wksItem of defaultWksItems) {
            if (wksItem.kind === 'line') {
                ...
                for (let i = 0; i < wksItem.repeat; i++) {
                    ...
                    const shape = { type: 'polygon', points: corners.map(p => ({ x: p.x, y: p.y })) };
                    items.push({
                        id,
                        layer: 'Frame',
                        kind: 'frame',
    ...
    paint(scene, renderer, layerState, highlightedIds = new Set(), netHighlightedIds = new Set(), 
    ...
    buildWireLike(wire, layer, color) {
        const points = typeof wire.getPoints === 'function' ? wire.getPoints() : [];
        if (points.length < 2) {
            return null;
        }
        ...
        const stroke = typeof wire.getStroke === 'function'
            ? wire.getStroke()
            : { width: 0.15, type: 'solid' };
        const width = stroke.width || 0.15;
        const lineType = (stroke.type ?? 'default');
    ...
    buildBus(bus) {
        ...
        const stroke = typeof bus.getStroke === 'function'
            ? bus.getStroke()
            : { width: 0.3, type: 'solid' };
        const width = stroke.width || 0.3;
        const lineType = (stroke.type ?? 'default');
        const s = item.shape;
        const shape = { ...item.shape, width };
        return {
    ...
    buildBusEntry(entry) {
        ...
        const shape = { type: 'segment', x1, y1, x2, y2, width };
        return {
    ...
    buildJunction(junction) {
        ...
        const shape = { type: 'circle', cx: origin.x, cy: origin.y, r: radius };
    ...
    buildNoConnect(nc) {
        ...
        const half = 0.9;
        const shape = { type: 'rect', x: origin.x - half, y: origin.y - half, w: half * 2, h: half * 2 };
     ...
     */
    buildSchPolyline(poly, forceClosed = false) {
    ...
    buildSchBezier(bezier) {
        const points = typeof bezier.getPoints === 'function' ? bezier.getPoints() : [];
        if (points.length !== 4) {
            return null;
        }
        const curve = cubicBezierToPolyline(new Vec2(points[0].x, points[0].y), new Vec2(points[1].x, points[1].y), new Vec2(points[2].x, points[2].y), new Vec2(points[3].x, points[3].y));
     ...
     * the image center. */
    buildSchImage(image, schematicVersion) {
        ...
        const shape = { type: 'rect', x, y, w: width, h: height, filled: true };
    ...
    buildRuleArea(ruleArea) {
        ...
        const shape = base.shape.type === 'polygon' ? { ...base.shape, filled: false } : base.shape;
        return {
     ...
     */
    buildSymbolInstance(instance, libSymbols) {
        const items = [];
        ...
        const placedUnit = typeof instance.getUnitId === 'function' ? instance.getUnitId() : 0;
        const instanceRef = typeof instance.getReference === 'function' ?
        ...
        if (typeof libDef.isDerived === 'function' &&
            libDef.isDerived() &&
        ...
        const unitCount = typeof unitCountSource.getUnitCount === 'function' ? unitCountSource.getUnitCount() :
            1;
        ...
        if (typeof instance.getVisibleProperties === 'function') {
            ...
            for (const name of Object.keys(visibleProps)) {
                ...
                const textWorld = symbolFieldWorldCenter(propOrigin, anchor, value, textSize, origin, mirror);
        ...
        if (isDnp) {
            ...
            if (Number.isFinite(pins.minX) && Number.isFinite(pins.minY) && pins.maxX > pins.minX && pins.maxY
                ...
                const markerShape = { type: 'polygon', points: points.map(p => ({ x: p.x, y: p.y })) };
                items.push({
    ...
    relevantSubUnits(libDef, placedUnit, libSymbols) {
        ...
        const subUnits = typeof graphicsSource.getLayers === 'function' ? graphicsSource.getLayers() : [];
    ...
    buildSymRect(rect, instanceMatrix, instanceId) {
        const { start, end } = rect.getStartEnd();
        const corners = [
    ...
    buildSymCircle(circle, instanceMatrix, instanceId) {
        const center = circle.getCenter();
        const radius = typeof circle.getRadius === 'function' ? circle.getRadius() : 0;
     ...
     */
    buildSymArc(arc, instanceMatrix, instanceId) {
        if (typeof arc.getArcCenterRadiusAngles !== 'function') {
            return null;
        }
        let local;
        ...
        let worldStartAngle, worldEndAngle;
        if (rawPts) {
    ...
    buildSymPolyline(poly, instanceMatrix, instanceId) {
        const points = typeof poly.getPoints === 'function' ? poly.getPoints() : [];
        ...
        const shape = { type: 'polygon', points: worldPoints.map(p => ({ x: p.x, y: p.y })) };
        return {
            id,
            layer: 'Symbols',
            kind: 'symbol-graphic',
    ...
    buildSchTextBox(textBox) {
        ...
        const margin = (index) => Number(margins[index]?.value) || 0;
        ...
        const strokeColor = strokeNode?.findFirstChildByName?.('color')?.getColor?.();
    ...
    buildSymBezier(bezier, instanceMatrix, instanceId) {
        const points = typeof bezier.getPoints === 'function' ? bezier.getPoints() : [];
        if (points.length !== 4) {
            return null;
        }
        const curve = cubicBezierToPolyline(flippedTransform(instanceMatrix, points[0].x, points[0].y), flippedTransform(instanceMatrix, points[1].x, points[1].y), flippedTransform(instanceMatrix, points[2].x, points[2].y), flippedTransform(instanceMatrix, points[3].x, points[3].y));
        const { width, type: lineType } = typeof bezier.getStroke === 'function' ? bezier.getStroke() :
            { width: 0.25, type: 'solid' };
     ...
     */
    buildSymText(text, instanceMatrix, instanceId) {
     ...
     */
    buildPin(pin, instanceMatrix, instanceId, pinNumbersHidden = false, pinNamesHidden = false, pinNameOffset = 0.508, refDesignator) {
        const items = [];
        ...
        if (name && name !== '~' && !namesHidden) {
            ...
            const hAlignAnchor = { left: 0, center: 0.5, right: 1 };
            const vAlignAnchor = { top: 0, center: 0.5, bottom: 1 };
     ...
     * depending on `shape`).
     */
    buildGlobalLabel(label) {
        const items = [];
        const name = typeof label.getName === 'function' ? label.getName() : '';
        if (!name) {
            return items;
        }
        const origin = label.getOrigin();
        const rotation = origin.rotation ?? 0;
        const shape = typeof label.getShape === 'function' ? label.getShape() : 'input';
        const { size: textSize, thickness, italic } = readElementFontMetrics(label);
        ...
        const flagShape = { type: 'polygon', points: worldPts.map(p => ({ x: p.x, y: p.y })) };
        items.push({
    ...
    buildHierLabel(label) {
        ...
        return this.buildHierLabelShape(id, name, new Vec2(origin.x, origin.y), rotation, shape, textSize, thickness, anchor.x, label, schColors.labelHier, true, italic);
    }
    /**
     ...
     */
    buildHierLabelShape(id, text, worldOrigin, rotation, shape, textSize, thickness, hAlign, element, color, 
     ...
     */
    buildNetclassFlag(flag) {
        const items = [];
        const origin = flag.getOrigin();
        const rotation = origin.rotation ?? 0;
        const shape = typeof flag.getShape === 'function' ? flag.getShape() : 'round';
    ...
    buildSheet(sheet) {
        ...
        if (getPinClass()) {
            for (const pin of sheet.findChildrenByClass(getPinClass())) {
                const pinName = pin.attributes?.[0]?.value;
                if (!pinName) {
                    continue;
                }
                const pinOrigin = typeof pin.getOrigin === 'function' ? pin.getOrigin() : { x, y, rotation: 0 };
                const pinShape = pin.attributes?.[1]?.value ?? 'passive';
                ...
                for (const item of this.buildHierLabelShape(markerId, pinName, new Vec2(pinOrigin.x, pinOrigin.y), pinOrigin.rotation ?? 0, sheetPinShape, pinTextSize, pinThickness, anchor.x, pin, schColors.sheetLabel, true, false, 'sheet-pin')) {
                    items.push(item);
                }
            }
        }
    ...
    buildTable(table) {
        ...
        }).filter((cell) => cell.colSpan > 0 && cell.rowSpan > 0 && cell.w > 0 && cell.h > 0);
        ...
        const lines = [];
        const addLine = (a, b, stroke) => lines.push({ a, b, stroke });
        ...
        items.push({
            ...
            draw: (renderer, color) => {
                for (const line of lines) {
                    ...
                    strokeDashedPolyline([line.a, line.b], line.stroke.width, line.stroke.type, segment => renderer.line(segment, { strokeColor: lineColor, strokeWidth: line.stroke.width }));
                }
            }
        });
        return items;
     ...
     */
    buildDanglingFlags(layerBuckets) {
        ...
        const isDangling = (x, y) => (occupants.get(pointKey(x, y)) ?? 0) <= 1;
        ...
        // to tell them apart.
        const liesOnAnyBusSpan = (x, y) => buses.some(bus => bus.shape.type === 'segment'
        ...
        const liesOnAnyRuleAreaBorder = (x, y) => ruleAreaPolygons.some(shape => polygonEdgeDistance(shape.points, shape.closed, x, y) < JUNCTION_POINT_EPS);
        ...
        const wireAndBusSegments = wireLike.filter(it => (it.kind === 'wire' || it.kind === 'bus') && it.shape.type === 'segment');
        const liesOnAnyWireOrBusSpan = (x, y) => wireAndBusSegments.some(seg => seg.shape.type === 'segment'
...
function danglingSquare(id, x, y, color, element) {
    const half = DANGLING_SQUARE_HALF;
    const shape = { type: 'rect', x: x - half, y: y - half, w: half * 2, h: half * 2 };
    return {
        id, layer: 'Dangling', kind: 'dangling', shape, bbox: shape, hitTestable: false, element,
        defaultColor: color,
        draw: (renderer, drawColor) => {
            renderer.rect(new Vec2(x - half, y - half), half * 2, half * 2, { strokeColor: drawColor, strokeWidth: DANGLING_STROKE_WIDTH });
        }
    };
}
function danglingCircle(id, x, y, color, element) {
    const shape = { type: 'circle', cx: x, cy: y, r: DANGLING_CIRCLE_RADIUS };
    return {
 ...
 * compensate for here. */
function rotateLocalPoint(p, rotationDeg) {
...
function drawStrokeOutline(renderer, ring, width, lineType, color) {
...
function cubicBezierToPolyline(start, control1, control2, end) {
    const points = [start];
    const midpoint = (a, b) => new Vec2((a.x + b.x) / 2, (a.y + b.y) / 2);
    const flatten = (a, b, c, d, depth) => {
        const flatness = Math.max(distanceToSegment(b.x, b.y, a.x, a.y, d.x, d.y), distanceToSegment(c.x, c.y, a.x, a.y, d.x, d.y));
...
function boldPenWidth(sizeMm) {
    return sizeMm / 5;
}
function readElementFontMetrics(el) {
 ...
 */
function labelClearanceOffset(textSize, thickness, rotationDeg) {
...
function textItem(id, layer, worldPos, textSize, element, geometry, defaultColor) {
    const bbox = getStrokeTextBounds(geometry);
    return {
 ...
 */
function wrapTableCellText(value, maxWidthMm, textSizeMm) {
    if (!(maxWidthMm > 0) || !value) {
        return value;
    }
    const fits = (text) => measureStrokeTextSize(text, textSizeMm).width <= maxWidthMm + 1e-6;
    const lines = [];
...
function letterSubReference(unit, initialLetter = 'A') {
    let suffix = '';
    let u = unit;
    do {
        const rem = (u - 1) % 26;
 ...
 */
function pinNamesHiddenFromEl(pinNamesEl) {
    if (!pinNamesEl) {
        return false;
    }
    ...
    if (Array.isArray(pinNamesEl.attributes)) {
        return pinNamesEl.attributes.some((a) => a.value === 'hide' || a.value === true);
    }
    return false;
}
...
function readNumericValue(el, fallback) {
    if (!el) {
        return fallback;
    }
...
function readPinChildFontSize(pin, childName, fallback) {
...
function readPinChildFontThickness(pin, childName, fallback) {
    const child = typeof pin.findFirstChildByName === 'function' ? pin.findFirstChildByName(childName) : null;
    const effects = child && typeof child.findFirstChildByName === 'function' ? child.findFirstChildByName('effects') :
        null;
    const font = effects && typeof effects.findFirstChildByName === 'function' ? effects.findFirstChildByName('font') :
...
function pinOrientationFromDir(ux, uy) {
 ...
 */
function orientPinOffset(local, orientation) {
    switch (orientation) {
        case 'right':
...
function uprightTextAngle(angleDeg) {
...
function buildInstanceMatrix(x, y, rotationDeg, mirror) {
 ...
 * wrong axis.
 */
function flippedTransform(instanceMatrix, localX, localY) {
...
function arcSweepAngles(center, startPt, midPt, endPt) {
    ...
    const forward = (from, to) => normalize(to - from);
 ...
 */
function symbolFieldWorldCenter(propOrigin, anchor, value, textSize, symbolOrigin, symbolMirror) {
...
function fieldDrawRotation(propRotationDeg, symbolRotationDeg) {
 ...
 */
function symbolFillColor(fillType, strokeColor, customColor) {
    switch (fillType) {
        case 'outline':
            return strokeColor;
        case 'background':
...
function dnpDimmedColor(value) {
    ...
    const bg = [40, 44, 52];
    const mix = (channel, background) => Math.round((channel + background) / 2);
...
let _Wire, _Bus, _BusEntry, _Junction, _NoConnect, _Symbol, _LibSymbols;
let _GlobalLabel, _HierLabel, _Sheet, _Table, _Image, _Pin, _NetclassFlag, _RuleArea;
let _Rect, _SymCircle, _SymArc, _Polyline, _Bezier, _At, _Size, _Text, _TextBox;
