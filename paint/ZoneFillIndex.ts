...
/**
 * Spatial index for a zone-fill polygon, mirroring KiCad's CN_ZONE_LAYER
 * (pcbnew/connectivity/connectivity_items.h): the fill outline is
 * triangulated once (ear clipping) and each triangle's bbox is inserted
 * into a uniform grid, so "does a point lie in the fill" becomes a grid
 * lookup + one barycentric point-in-triangle test instead of the O(V) walk
 * pcbnew-equivalent `pointInPolygon` does. This is the exact change that
 * makes KiCad's zone-item / zone-zone connectivity instant where KiOnline
 * was doing an O(fill_vertices × same-net items) sweep.
 *
 * Attribution: the ear-clipping triangulation is a standard algorithm (this
 * file's own original implementation), not a port — its behavior is designed
 * to be equivalent to the existing `pointInPolygon(fill.points, x, y)`
 * inside/outside semantics already used by BoardCopperGraph.ts.
 *
 * If the outline can't be triangulated (degenerate, or a self-touching
 * keyhole-bridged ring), the index falls back to exact pointInPolygon so a
 * broken/edge-case polygon never changes connectivity: correctness first,
 * speed second.
 */
export interface ZoneFillIndex {
	readonly bbox: { x: number; y: number; w: number; h: number };
	readonly points: { x: number; y: number }[];
	/** Exact point-in-fill test, O(V) worst case (fallback) but O(1)-ish for
	 *  well-formed triangulated fills. Uses the same winding semantics as
	 *  pointInPolygon. */
	containsPoint(x: number, y: number): boolean;
	/** Does a (small/simple) shape polygon overlap the filled region? Uses the
	 *  fill's edge grid (KiCad's SHAPE::Collide-style outline test) so it is
	 *  near-O(shape edges × local fill edges) instead of an O(V_fill) sweep. */
	overlapsPolygon(otherPoints: { x: number; y: number }[], closed: boolean): boolean;
	/** Does a stroked segment (a track) touch the filled region, honoring the
	 *  segment's width exactly like shapesOverlap's segment-vs-polygon test? */
	segmentTouches(x1: number, y1: number, x2: number, y2: number, width: number): boolean;
}
...
function segmentsTouch(
	...
	const cross = (px1: number, py1: number, px2: number, py2: number, px3: number, py3: number): number =>
 ...
 */
const zoneIndexCache = new WeakMap<object, ZoneFillIndex>();

export function buildZoneFillIndex(points: { x: number; y: number }[]): ZoneFillIndex | null {
	...
	function containsPointFromTriangles(x: number, y: number): boolean {
		...
		for (const t of bucket) {
			if (barycentricInTri(t, x, y)) return true;
		}
		return false;
	}

	function outerBBoxReject(x: number, y: number): boolean {
	...
	let fallbackGrid: { cell: number; boundary: Set<string>; inside: Set<string> } | null = null;
	function buildFallbackGrid(): void {
		...
		for (let cx = minCx; cx <= maxCx; cx++) {
			for (let cy = minCy; cy <= maxCy; cy++) {
				...
				if (pointInPolygon(points, cxp, cyp)) inside.add(key);
			}
		}
		fallbackGrid = { cell: triCellMM, boundary, inside };
	}
	function containsPointFallback(x: number, y: number): boolean {
		if (!outerBBoxReject(x, y)) return false;
		if (!fallbackGrid) {
			...
			if (points.length >= 64) buildFallbackGrid();
			else fallbackGrid = null;
		}
		if (fallbackGrid) {
	...
	const outlineEdges: { a: { x: number; y: number }; b: { x: number; y: number }; minX: number; minY: number; maxX: number; maxY: number }[] = [];
	const edgeGrid = new Map<string, typeof outlineEdges>();
	if (points.length >= 2) {
		for (let i = 0; i < points.length; i++) {
			const a = points[i]!, b = points[(i + 1) % points.length]!;
			const e = {
	...
	function overlapsPolygon(other: { x: number; y: number }[], closed: boolean): boolean {
		...
		const edgeSeen = new Set<typeof outlineEdges[number]>();
		for (let cx = minCx; cx <= maxCx; cx++) {
			for (let cy = minCy; cy <= maxCy; cy++) {
				for (const fe of edgeGrid.get(`${ cx }\u0000${ cy }`) ?? []) {
					if (edgeSeen.has(fe)) continue;
					edgeSeen.add(fe);
					...
					for (let i = 0; i < edgeCount; i++) {
						...
						if (fe.maxX < oMinX - 1e-9 || oMaxX < fe.minX - 1e-9
							|| fe.maxY < oMinY - 1e-9 || oMaxY < fe.minY - 1e-9) continue;
						if (segmentsTouch(a.x, a.y, b.x, b.y, fe.a.x, fe.a.y, fe.b.x, fe.b.y)) return true;
					}
				}
			}
		}
		if (!hasTriangles) {
			...
			for (const fp of points) if (pointInPolygon(other, fp.x, fp.y)) return true;
		}
		return false;
	}
	...
	const radius = (shapeSpaceWidth: number): number => shapeSpaceWidth / 2 + 1e-9;
	function segmentTouches(x1: number, y1: number, x2: number, y2: number, width: number): boolean {
		...
		const seen = new Set<typeof outlineEdges[number]>();
		for (let cx = minCx; cx <= maxCx; cx++) {
			for (let cy = minCy; cy <= maxCy; cy++) {
				for (const fe of edgeGrid.get(`${ cx }\u0000${ cy }`) ?? []) {
					...
					if (fe.minX - r > segMaxX || segMinX > fe.maxX + r
						|| fe.minY - r > segMaxY || segMinY > fe.maxY + r) continue;
					// A crossing (segment goes through the fill outline) or the
					// stroked body reaching the edge — mirror segmentTouchesPolygon.
					if (segmentsTouch(x1, y1, x2, y2, fe.a.x, fe.a.y, fe.b.x, fe.b.y)) return true;
					if (distanceToSegment(fe.a.x, fe.a.y, x1, y1, x2, y2) <= r) return true;
					if (distanceToSegment(fe.b.x, fe.b.y, x1, y1, x2, y2) <= r) return true;
					if (distanceToSegment(x1, y1, fe.a.x, fe.a.y, fe.b.x, fe.b.y) <= r) return true;
					if (distanceToSegment(x2, y2, fe.a.x, fe.a.y, fe.b.x, fe.b.y) <= r) return true;
				}
			}
		}
		return false;
	...
	const index: ZoneFillIndex = { bbox, points, containsPoint, overlapsPolygon, segmentTouches };
...
function barycentricInTri(t: Tri, px: number, py: number): boolean {
 ...
 */
function triangulateRing(pts: { x: number; y: number }[]): [{ x: number; y: number }, { x: number; y: number }, { x: number; y: number }][] | null {
	const n = pts.length;
	if (n < 3) return null;
	...
	const triangles: [TriPt, TriPt, TriPt][] = [];
	let guard = 0;
