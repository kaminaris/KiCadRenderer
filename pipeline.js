...
export function rebuildActiveScene(session) {
...
export function scheduleFootprintRebuild(session, footprint) {
...
export function rebuildAfterFootprintGeometryEdit(session, footprint) {
    if (session.dragPreviewFootprints.has(footprint)) {
        session.dragPreviewFootprints.set(footprint, session.painter.buildFootprintPreviewItems(session.boardRoot, footprint));
...
export function rebuildSchScene(session) {
...
export function rebuildBoardSceneIfPending(session) {
