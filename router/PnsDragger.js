...
export class Direction45 {
    dir;
    is90;
    constructor(dir, is90) {
        this.dir = dir;
        this.is90 = is90;
    }
    static fromVector(vx, vy, is90) {
        if (vx === 0 && vy === 0) {
            return new Direction45(-1 /* Dir.UNDEFINED */, is90);
        }
        const flippedY = -vy;
        ...
        if (dir < 0)
            dir += 8;
        return new Direction45(dir, is90);
    }
    static fromSegment(a, b, is90) {
        return Direction45.fromVector(b.x - a.x, b.y - a.y, is90);
    }
    get isDefined() { return this.dir !== -1 /* Dir.UNDEFINED */; }
    equals(other) { return this.dir === other.dir; }
    right() {
        if (this.dir === -1 /* Dir.UNDEFINED */)
            return this;
        const step = this.is90 ? 2 : 1;
        return new Direction45(((this.dir + step) % 8), this.is90);
    }
    left() {
        if (this.dir === -1 /* Dir.UNDEFINED */)
            return this;
        const step = this.is90 ? 2 : 1;
        return new Direction45(((this.dir + 8 - step) % 8), this.is90);
    }
    ...
    isObtuseOrHalfFull(other) {
        if (this.dir === -1 /* Dir.UNDEFINED */ || other.dir === -1 /* Dir.UNDEFINED */) {
            return false;
        }
     ...
     *  index: NE/SE/SW/NW). */
    isDiagonal() {
        return this.dir !== -1 /* Dir.UNDEFINED */ && (this.dir % 2) === 1;
    }
    toVector() {
        switch (this.dir) {
            case 0 /* Dir.N */: return new Vec2(0, -1);
            case 4 /* Dir.S */: return new Vec2(0, 1);
            case 2 /* Dir.E */: return new Vec2(1, 0);
            case 6 /* Dir.W */: return new Vec2(-1, 0);
            case 1 /* Dir.NE */: return new Vec2(1, -1);
            case 7 /* Dir.NW */: return new Vec2(-1, -1);
            case 3 /* Dir.SE */: return new Vec2(1, 1);
            case 5 /* Dir.SW */: return new Vec2(-1, 1);
...
function intersectLinesInfinite(l1, l2) {
...
function intersectSegmentsBounded(s1, s2) {
 ...
 */
export function dragSegment45(points, segmentIndex, target, cornerMode) {
    ...
    const dragged = { a: path[index], b: path[index + 1] };
    const dragDir = Direction45.fromSegment(dragged.a, dragged.b, is90);
    let sPrev = { a: path[index - 1], b: path[index] };
    let sNext = { a: path[index + 1], b: path[index + 2] };
    ...
    const draggedNow = { a: path[index], b: path[index + 1] };
    const guideA = segmentIndex === 0
        ...
        : dirPrev.isObtuseOrHalfFull(dragDir)
            ...
            : [{ a: draggedNow.a, b: addVec(draggedNow.a, dirPrev.toVector()) },
                { a: draggedNow.a, b: addVec(draggedNow.a, dirPrev.toVector()) }];
    const guideB = segmentIndex === points.length - 2
        ...
        : dirNext.isObtuseOrHalfFull(dragDir)
            ...
            : [{ a: draggedNow.b, b: addVec(draggedNow.b, dirNext.toVector()) },
                { a: draggedNow.b, b: addVec(draggedNow.b, dirNext.toVector()) }];
    const sCurrent = { a: target, b: addVec(target, dragDir.toVector()) };
    ...
    for (let i = 0; i < 2; i++) {
        for (let j = 0; j < 2; j++) {
            ...
            const s1 = { a: sPrev.a, b: ip1 };
            const s3 = { a: ip2, b: sNext.b };
            let candidate;
 ...
 */
export function buildInitialTrace(p0, p1, cornerMode, startDiagonal = false) {
 ...
 */
export function dragViaChain(originPoints, target, cornerMode) {
    ...
    const dirLast = Direction45.fromSegment(originPoints[originPoints.length - 2], originPoints[originPoints.length - 1], false);
