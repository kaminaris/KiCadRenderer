 ...
 */
export function pushUndoSnapshot(session, label = 'Edit') {
    ...
    const max = (session.constructor && session.constructor.maxUndoDepth) || 100;
...
export function clearUndoRedo(session) {
...
export async function undo(session) {
    if (!canUndo(session))
        return false;
...
export async function redo(session) {
    if (!canRedo(session))
        return false;
    const current = session.documentType === 'schematic' ? session.getSchematicText() : session.getBoardText();
...
export async function cancelLatestUndoSnapshot(session) {
...
export function getUndoStackDebug(session) {
    return {
        undoDepth: session.undoStack ? session.undoStack.length : 0,
        redoDepth: session.redoStack ? session.redoStack.length : 0,
        undo: (session.undoStack || []).map((text, index) => ({
            label: (session.undoLabels && session.undoLabels[index]) || 'Edit',
            bytes: text.length
        }))
    };
}
export function beginBatch(session) {
    if (!session)
        return;
    if (typeof session.batchDepth !== 'number')
        session.batchDepth = 0;
    session.batchDepth++;
}
export function endBatch(session) {
    if (!session)
        return;
    if (typeof session.batchDepth !== 'number')
        session.batchDepth = 0;
...
export function commitAstMutation(session) {
