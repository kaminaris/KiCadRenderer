...
// Query a small box; verify every result actually overlaps it
const qmin = [0, 500, 500];
const qmax = [0, 520, 520];
const hits: number[] = [];
rtree.Search(qmin, qmax, (d) => { hits.push(d); return true; });
// verify no false positives by checking each hit's stored box (recompute via a naive scan)
let expected = 0;
for (let i = 0; i < N; i++) {
	const x = i % 2 === 0 ? (i * 7) % 1000 : (i * 13) % 1000; // not exact — recompute needs stored boxes
}
// Simpler: re-insert into a second tree keyed the same way and compare naive scan
const boxes: number[][] = [];
...
for (let i = 0; i < N; i++) {
	const b = boxes[i]!;
	if (b[0]! <= qmax[0]! && b[3]! >= qmin[0]! && b[1]! <= qmax[1]! && b[4]! >= qmin[1]! && b[2]! <= qmax[2]! && b[5]! >= qmin[2]!) {
		naive.push(i);
	}
}
