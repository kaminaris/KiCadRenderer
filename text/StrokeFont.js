...
export class StrokeFont {
    static overbarPositionFactor = 1.4;
    static underlinePositionFactor = -0.16;
    static fontScale = 1 / 21;
    static fontOffset = -10;
    static italicTilt = 1.0 / 8;
    static interlinePitchRatio = 1.62;
    static instance;
    static default() {
        if (!this.instance) {
    ...
    loadGlyph(idx) {
        const data = newstroke.glyphData[idx];
    ...
    computeUnderlineVerticalPosition(glyphHeight) {
        return glyphHeight * StrokeFont.underlinePositionFactor;
    }
    computeOverbarVerticalPosition(glyphHeight) {
        return glyphHeight * StrokeFont.overbarPositionFactor;
    }
    getInterline(glyphHeight, lineSpacing = 1) {
     ...
     */
    getTextAsGlyphs(text, size, position, angle, mirror, origin, style = {}) {
        ...
        for (const c of text) {
            if (c === '\t') {
                ...
                glyphs.push(source.transform(glyphSize, cursor, tilt, angle, mirror, origin));
                if (tilt) {
        ...
        if (hasBar) {
            ...
            const barStart = new Vec2(position.x + barOffset.x + barTrim, cursor.y - barOffset.y);
            const barEnd = new Vec2(cursor.x + barOffset.x - barTrim, cursor.y - barOffset.y);
            const barGlyph = new StrokeGlyph([[barStart, barEnd]], BBox.fromPoints([barStart, barEnd]));
            glyphs.push(barGlyph.transform(new Vec2(1, 1), new Vec2(0, 0), 0, angle, mirror, origin));
        }
        const bbox = new BBox();
        bbox.start = position;
        bbox.end = new Vec2(cursor.x + barOffset.x - glyphSize.x * interChar, cursor.y + Math.max(glyphSize.y, barOffset.y * StrokeFont.overbarPositionFactor));
        return { bbox, glyphs, cursor: new Vec2(cursor.x, position.y) };
    }
}
function decodeCoordVal(c) {
 ...
 */
function decodeGlyph(glyphData) {
    let startX = 0;
    let width = 0;
    let minY = 0;
    let maxY = 0;
    ...
    for (let i = 0; i < glyphData.length; i += 2) {
        ...
        if (i < 2) {
            ...
            const point = new Vec2(decodeCoordVal(c0) * StrokeFont.fontScale - startX, (decodeCoordVal(c1) + StrokeFont.fontOffset) * StrokeFont.fontScale);
            if (points == null) {
                points = [];
                strokes.push(points);
            }
