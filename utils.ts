...
export function sexprParenDelta(line: string): number {
	let delta = 0;
	let inQuote = false;
...
export function repairLegacyMalformedZoneText(text: string): string {
...
export function resolveZoneClearanceMm(zone: any, designSettings?: any): number {
	return Math.max(
		zone.getClearance(),
...
export function arcCircumcenter(
...
export function quantizedAngle(fromX: number, fromY: number, toX: number, toY: number): number {
...
export function pointLiesOnSegmentInterior(px: number, py: number, x1: number, y1: number, x2: number, y2: number): boolean {
...
export function cubicBezierToPolyline(p0: any, p1: any, p2: any, p3: any, steps = 32): any[] {
	const points: any[] = [];
...
export function computeWireBend(from: { x: number; y: number }, to: { x: number; y: number }, mode: 'free' | '90' | '45'): { x: number; y: number } | null {
